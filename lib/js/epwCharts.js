...
function epwHCatChart() {
    ...
    function chart(div) {
        div.each(function() {
            ...
            // Display order: highest key (hottest stress) at top, key 0 at bottom.
...
function epwScatterChart(epw) {
    ...
    // Chart geometry
    var margin = { top: 34, right: 30, bottom: 60, left: 120 };
    var width  = 720;
    var height = 430;
    ...
    function redrawAxes() {
        ...
        if (yVd.catLabels) { _wrapCatAxisLabels(_scYAxisApplied); }
...
function epwTimeseriesCharts(epw) {
    ...
    function drawAnnualTimeseries(activeData, varDef) {
        ...
        var margin  = { top: 34, right: 20, bottom: 40, left: varDef.catLabels ? 150 : 62 };
        var pxPerDay = 2.5;
        ...
        var yAxis = varDef.catLabels
            ? d3.svg.axis().scale(yScale).orient('left')
                .tickValues(d3.range(0, varDef.catLabels.length))
        ...
        if (varDef.catLabels) { _wrapCatAxisLabels(_tsYAxisG); }
    ...
    function drawDailyProfiles(activeData, varDef) {
        ...
        var panelInnerH  = 170;
        var panelGutter  = 4;
        var marginLeft   = varDef.catLabels ? 150 : 56;   // space for shared y-axis
        ...
        var _dpYAxisG = svg.append('g')
            ...
            .call(yAxis);
        if (varDef.catLabels) { _wrapCatAxisLabels(_dpYAxisG); }
...
function epwDurationCurve(epw) {
  ...
  function draw(activeData) {
    ...
    var margin = { top: 24, right: 165, bottom: 60, left: vDef.catLabels ? 160 : 70 };
    ...
    var yAxis = vDef.catLabels
        ? d3.svg.axis().scale(yScale).orient('left')
            .tickValues(d3.range(0, vDef.catLabels.length))
    ...
    var _durYAxisG = svg.append('g')
        .attr('class', 'y axis')
        .call(yAxis);
    if (vDef.catLabels) { _wrapCatAxisLabels(_durYAxisG); }
