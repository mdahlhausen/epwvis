...
function epwDegreeDays(epw) {
  ...
  function draw(activeData) {
      ...
      [
        ...
        if (bar.filtVal > 0) {
          ...
          svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw / 2)
              .attr('y', yScale(bar.filtVal) - 3)
              .attr('text-anchor', 'middle')
              .style('font-size', '11px')
              .style('fill', bar.colorFilt)
              .text(Math.round(bar.filtVal));
        }
      });
    });
    ...
    svg.append('text')
        ...
        .text('HDD(' + hBase + uLabel + '): ' + Math.round(totalHDD) + '  |  CDD(' + cBase + uLabel + '): ' + Math.round(totalCDD));

    // SVG-embedded legend (4 swatches in a horizontal row)
    var ddLegG = svg.append('g')
        .attr('data-epv-legend', '1')
        .attr('transform', 'translate(' + (W / 2 - 200) + ',-16)');
    [
      { color: HDD_FILT, label: 'HDD (filtered)', x: 0   },
      { color: HDD_ALL,  label: 'HDD (all data)', x: 110  },
      { color: CDD_FILT, label: 'CDD (filtered)', x: 222  },
      { color: CDD_ALL,  label: 'CDD (all data)', x: 332  }
  ...
  function drawByTemp(activeData) {
    ...
    document.getElementById('ddByTempTotals').innerHTML =
      '<span class="dd-hdd">Heating Degree Days (' + hBase + uLabel + '): ' + Math.round(totalHDD2) + '</span>' +
      '&nbsp;&nbsp;&nbsp;&nbsp;' +
      '<span class="dd-cdd">Cooling Degree Days (' + cBase + uLabel + '): ' + Math.round(totalCDD2) + '</span>';

    var margin2 = { top: 50, right: 30, bottom: 50, left: 75 };
    var svg2W = 820, svg2H = 280;
    ...
    bins.forEach(function(b) {
      ...
      if (hddFilt > 0) {
        ...
        if (Math.round(hddFilt) > 0) {
          svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw / 2).attr('y', yScale(hddFilt) - 3)
              .attr('text-anchor', 'middle')
              .style('font-size', '9px').style('fill', HDD_FILT)
              .text(Math.round(hddFilt));
        }
      }
      ...
      if (cddFilt > 0) {
        ...
        if (Math.round(cddFilt) > 0) {
          svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw + 1 + bw / 2).attr('y', yScale(cddFilt) - 3)
              .attr('text-anchor', 'middle')
              .style('font-size', '9px').style('fill', CDD_FILT)
              .text(Math.round(cddFilt));
        }
      }
    });

    // X-axis: label every other bin to avoid crowding
    var xAxis = d3.svg.axis().scale(xScale).orient('bottom')
        .tickFormat(function(b, i) { return i % 2 === 0 ? b + uLabel : ''; });
    ...
    svg.append('text')
        .attr('data-epv-datalabel', '1')
        .attr('x', W2 / 2).attr('y', H2 + 40)
        .attr('text-anchor', 'middle')
        .style('font-size', '11px').style('fill', '#555')
        .text('HDD(' + hBase + uLabel + '): ' + Math.round(totalHDD2) + '  |  CDD(' + cBase + uLabel + '): ' + Math.round(totalCDD2));
    ...
    // Legend (matches monthly chart style)
    var ddLegG2 = svg.append('g')
        .attr('data-epv-legend', '1')
        .attr('transform', 'translate(' + (W2 / 2 - 200) + ',-14)');
    [
      { color: HDD_FILT, label: 'HDD (filtered)', x: 0   },
      { color: HDD_ALL,  label: 'HDD (all data)', x: 110 },
      { color: CDD_FILT, label: 'CDD (filtered)', x: 222 },
