...
function epwValueFilter(epw) {
	...
	var	dryBulbTemperatureMin = Math.min.apply(Math,tempArray), 
		...
		dewPointTemperature = xfilter.dimension(function(d) { return d.dewPointTemperature; }),
		tempArray = epw.dewPointTemperature();
		if (unitSystem == "IP") { tempArray = convertCtoF(tempArray) };
	...
	if (unitSystem == "IP") {
		dryBulbTemperatures = dryBulbTemperature.group(function(d) { return Math.floor(d); }); // (d / 2) * 2
		dewPointTemperatures = dewPointTemperature.group(function(d) { return Math.floor(d); }); // (d / 2) * 2
	} else {
    ...
    var charts = [
        ...
        epwBarChart()
          ...
          .x(d3.scale.linear()
            .domain([1, 12.5])
            .rangeRound([0, 20 * 12.5])), //make sure pixel width plus margins matches epwvis.css
        ...
        epwBarChart()
          ...
          .x(d3.scale.linear()
            .domain([1, 25])
            .rangeRound([0, 20 * 24.5])), //make sure pixel width plus margins matches epwvis.css
        ...
        epwBarChart()
          ...
          .x(d3.scale.linear()
            ...
            .rangeRound([0, 10 * 90])), //make sure pixel width plus margins matches epwvis.css
