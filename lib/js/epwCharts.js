...
// Returns { altitude: [], azimuth: [] } for the loaded EPW, computed once.
// altitude in degrees (+ = above horizon), azimuth in degrees (N=0, clockwise).
function getSolarPositions(epw) {
    if (!window._solarPositionsEpw || window._solarPositionsEpw !== epw) {
        if (window.SolarPV && window.SolarPV.computeSolarPositions) {
            window._solarPositions    = window.SolarPV.computeSolarPositions(epw);
            window._solarPositionsEpw = epw;
        } else {
            window._solarPositions    = null;
            window._solarPositionsEpw = epw;
        }
    }
    return window._solarPositions;
}
...
function clearEPWCharts() {
    ...
    window.updateSunChart      = null;
    window._sunLastActive      = null;
    window._sunVarId           = null;
    window._solarPositions     = null;
    window._solarPositionsEpw  = null;
...
function epwDataDefault(epw,unitSystem){
	var month = epw.month(),
		...
		var solPos = getSolarPositions(epw);
		var solAlt = solPos ? solPos.altitude : null;
		var solAzm = solPos ? solPos.azimuth  : null;
		var utciArr = epw.utci();
	...
	for (var i=0; i < month.length; i++){
	  ...
	  datum = {"index":i,"month":month[i],"day":day[i],
			   ...
			   "solarAltitude": solAlt ? solAlt[i] : null,
			   "solarAzimuth":  solAzm ? solAzm[i] : null,
			   "utci": utciArr[i] !== undefined ? utciArr[i] : null,
			   "utciCategoryIndex": _utciCategoryIndex(utciArr[i] !== undefined ? utciArr[i] : null)};
	  data.push(datum);
	};
...
// Maps a UTCI value [°C] to a stress category index 0–10 (coldest → hottest).
// Returns null for null/non-finite values.
function _utciCategoryIndex(v) {
    if (v === null || !isFinite(v)) return null;
    if (v <  -40) return 0;  // extreme cold stress
    if (v <  -27) return 1;  // very strong cold stress
    if (v <  -13) return 2;  // strong cold stress
    if (v <    0) return 3;  // moderate cold stress
    if (v <    9) return 4;  // slight cold stress
    if (v <   18) return 5;  // no thermal stress (cool)
    if (v <   26) return 6;  // thermal comfort zone
    if (v <   32) return 7;  // moderate heat stress
    if (v <   38) return 8;  // strong heat stress
    if (v <   46) return 9;  // very strong heat stress
    return 10;               // extreme heat stress
}
var _utciCatNames = ['Extreme Cold','Very Strong Cold','Strong Cold','Moderate Cold','Slight Cold',
                     'No Thermal Stress (cool)','Thermal Comfort Zone','Moderate Heat','Strong Heat','Very Strong Heat','Extreme Heat'];
function _utciCatLabel(i) {
    i = Math.round(i);
    return (i >= 0 && i < _utciCatNames.length) ? _utciCatNames[i] : String(i);
}

/** Wrap long categorical y-axis tick labels into 2 lines with smaller font. */
function _wrapCatAxisLabels(axisG) {
    axisG.selectAll('.tick text').each(function() {
        ...
        el.text('');
        el.style('font-size', '9.5px');
        var mid = Math.ceil(words.length / 2);
        el.append('tspan').attr('x', -3).attr('dy', '-0.3em').text(words.slice(0, mid).join(' '));
        el.append('tspan').attr('x', -3).attr('dy', '1.2em').text(words.slice(mid).join(' '));
    });
}

// Single-line variant: shrinks font to fit without wrapping (for y-axes with limited width budget)
function _singleLineCatAxisLabels(axisG) {
    axisG.selectAll('.tick text')
        .style('font-size', '9px')
        .attr('text-anchor', 'end');
}

// Double-line variant for x-axis (bottom) categorical labels
function _wrapCatXAxisLabels(axisG) {
    axisG.selectAll('.tick text').each(function() {
        var el = d3.select(this);
        var label = el.text();
        if (!label) return;
        var words = label.split(' ');
        el.text('');
        el.style('font-size', '9px').attr('text-anchor', 'middle');
        var mid = Math.ceil(words.length / 2);
        el.append('tspan').attr('x', 0).attr('dy', '0.9em').text(words.slice(0, mid).join(' '));
        if (words.length > 1) {
...
/*-------------------------------------------------------------------------
 * epwVarDefs(isIP)
 * Canonical variable definition list shared by scatter, tabular, timeseries,
 * duration curve, and sun chart tabs.  Each entry exposes:
 *   id         — key on pre-converted data rows (d[id])
 *   label      — display label
 *   unitIP / unitSI  — unit strings; .unit is set to the active one
 *   isSolar    — skip zero-radiation/nighttime hours in statistics
 *   fmt        — optional value formatter fn(v)→string (tooltips, axes)
 *   tblFmt     — optional cell formatter for the tabular tab
 *   getVal     — fn(rawRow)→value for scatter (reads epwDataDefault rows)
 *-------------------------------------------------------------------------*/
function epwVarDefs(isIP) {
    var defs = [
        { id: 'dryBulbTemperature',         label: 'Dry Bulb Temperature',
          unitIP: '\u00B0F', unitSI: '\u00B0C',           isSolar: false,
          getVal: function(d) { return d.dryBulbTemperature; } },
        { id: 'dewPointTemperature',         label: 'Dew Point Temperature',
          unitIP: '\u00B0F', unitSI: '\u00B0C',           isSolar: false,
          getVal: function(d) { return d.dewPointTemperature; } },
        { id: 'wetBulbTemperature',          label: 'Wet Bulb Temperature',
          unitIP: '\u00B0F', unitSI: '\u00B0C',           isSolar: false,
          getVal: function(d) { return d.wetBulbTemperature; } },
        { id: 'relativeHumidity',            label: 'Relative Humidity',
          unitIP: '%',       unitSI: '%',                  isSolar: false,
          getVal: function(d) { return d.relativeHumidity; } },
        { id: 'humidityRatio',               label: 'Humidity Ratio',
          unitIP: 'gr/lb',   unitSI: 'g/kg',               isSolar: false,
          getVal: function(d) { return (d.humidityRatio !== null && isFinite(d.humidityRatio)) ? d.humidityRatio * (isIP ? 7000 : 1000) : null; } },
        { id: 'enthalpy',                    label: 'Enthalpy',
          unitIP: 'BTU/lb',  unitSI: 'kJ/kg',              isSolar: false,
          getVal: function(d) { var raw = d.moistAirEnthalpy; return (raw !== null && isFinite(raw)) ? raw * (isIP ? 4.29923e-4 : 0.001) : null; } },
        { id: 'globalHorizontalRadiation',   label: 'Global Horiz. Radiation',
          unitIP: 'Wh/m\u00B2', unitSI: 'Wh/m\u00B2',    isSolar: true,
          getVal: function(d) { return d.globalHorizontalRadiation; } },
        { id: 'directNormalRadiation',       label: 'Direct Normal Radiation',
          unitIP: 'Wh/m\u00B2', unitSI: 'Wh/m\u00B2',    isSolar: true,
          getVal: function(d) { return d.directNormalRadiation; } },
        { id: 'diffuseHorizontalRadiation',  label: 'Diffuse Horiz. Radiation',
          unitIP: 'Wh/m\u00B2', unitSI: 'Wh/m\u00B2',    isSolar: true,
          getVal: function(d) { return d.diffuseHorizontalRadiation; } },
        { id: 'windSpeed',                   label: 'Wind Speed',
          unitIP: 'knots',   unitSI: 'm/s',                isSolar: false,
        ...
        { id: 'totalSkyCover',               label: 'Total Sky Cover',
          unitIP: 'tenths',  unitSI: 'tenths',             isSolar: false,
          getVal: function(d) { return d.totalSkyCover; } },
        { id: 'opaqueSkyCover',              label: 'Opaque Sky Cover',
          unitIP: 'tenths',  unitSI: 'tenths',             isSolar: false,
          getVal: function(d) { return d.opaqueSkyCover; } },
        { id: 'visibility',                  label: 'Visibility',
          unitIP: 'km',      unitSI: 'km',                 isSolar: false,
          getVal: function(d) { return d.visibility; } },
        { id: 'ceilingHeight',               label: 'Ceiling Height',
          unitIP: 'ft',      unitSI: 'm',                  isSolar: false,
          getVal: function(d) { return d.ceilingHeight !== null ? d.ceilingHeight * (isIP ? 3.28084 : 1) : null; } },
        { id: 'precipitableWater',           label: 'Precipitable Water',
          unitIP: 'mm',      unitSI: 'mm',                 isSolar: false,
          getVal: function(d) { return d.precipitableWater; } },
        { id: 'aerosolOpticalDepth',         label: 'Aerosol Optical Depth',
          unitIP: '',        unitSI: '',                   isSolar: false,
          fmt: function(v) { return Number(v).toPrecision(3); },
          getVal: function(d) { return d.aerosolOpticalDepth; } },
        { id: 'snowDepth',                   label: 'Snow Depth',
          unitIP: 'in',      unitSI: 'cm',                 isSolar: false,
          getVal: function(d) { return d.snowDepth !== null ? d.snowDepth * (isIP ? 0.393701 : 1) : null; } },
        { id: 'daysSinceLastSnowfall',       label: 'Days Since Last Snowfall',
          unitIP: 'days',    unitSI: 'days',               isSolar: false,
          getVal: function(d) { return d.daysSinceLastSnowfall; } },
        { id: 'albedo',                      label: 'Albedo',
          unitIP: '',        unitSI: '',                   isSolar: false,
          fmt: function(v) { return Number(v).toPrecision(3); },
          getVal: function(d) { return d.albedo; } },
        { id: 'liquidPrecipitationDepth',    label: 'Liquid Precip. Depth',
          unitIP: 'mm',      unitSI: 'mm',                 isSolar: false,
          getVal: function(d) { return d.liquidPrecipitationDepth; } },
        { id: 'liquidPrecipitationQuantity', label: 'Liquid Precip. Quantity',
          unitIP: 'hr',      unitSI: 'hr',                 isSolar: false,
          getVal: function(d) { return d.liquidPrecipitationQuantity; } },
        { id: 'pressure',                    label: 'Station Pressure',
          unitIP: 'inHg',    unitSI: 'Pa',                 isSolar: false,
          getVal: function(d) { var raw = d.atmosphericStationPressure; return (raw !== null && isFinite(raw)) ? raw * (isIP ? 2.95299830714e-4 : 1) : null; } },
        { id: 'pvAC',                        label: 'PV AC Output',
          unitIP: 'kWh',     unitSI: 'kWh',                isSolar: true,
        ...
        { id: 'solarAltitude',               label: 'Solar Altitude',
          unitIP: '\u00B0',  unitSI: '\u00B0',             isSolar: true,
          getVal: function(d) { return d.solarAltitude; } },
        { id: 'solarAzimuth',                label: 'Solar Azimuth',
          unitIP: '\u00B0',  unitSI: '\u00B0',             isSolar: false,
          getVal: function(d) { return d.solarAzimuth; } },
        { id: 'utci',                        label: 'UTCI',
          unitIP: '\u00B0C', unitSI: '\u00B0C',            isSolar: false,
          getVal: function(d) { return d.utci; } },
        { id: 'utciCategoryIndex',           label: 'UTCI Stress Category',
          unitIP: '',        unitSI: '',                   isSolar: false,
          fmt:    _utciCatLabel,
          tblFmt: function(v) { return (v === null || !isFinite(v)) ? '\u2014' : _utciCatLabel(v); },
          catLabels: _utciCatNames,
          colors: ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#4dcfb0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'],
          getVal: function(d) { return d.utciCategoryIndex; } }
    ];
    defs.forEach(function(v) { v.unit = isIP ? v.unitIP : v.unitSI; });
    return defs;
}
...
function epwHeatmapCharts(epw) {
    ...
    var hmVarDefs = [
        ...
        { id: 'pvACOutput',
          ...
          autoMinMax: false, fixMin: 0, fixMax: null },
        { id: 'solarAltitude',
          label: 'Solar Altitude',             defaultChecked: false,
          getVal: function(e, ip) {
              var sp = window._solarPositions;
              if (!sp) return new Array(e.dryBulbTemperature().length).fill(null);
              return sp.altitude.slice();
          },
          unit: function(ip) { return '°'; },
          steps: 9, colors: ['#000033','#003399','#0066ff','#66ccff','#ffff00','#ff9900','#ff6600','#ff3300','#cc0000','#990000'],
          autoMinMax: false, fixMin: -20, fixMax: 90 },
        { id: 'solarAzimuth',
          label: 'Solar Azimuth',              defaultChecked: false,
          getVal: function(e, ip) {
              var sp = window._solarPositions;
              if (!sp) return new Array(e.dryBulbTemperature().length).fill(null);
              return sp.azimuth.slice();
          },
          unit: function(ip) { return '°'; },
          steps: 8, colors: ['#5e4fa2','#3288bd','#66c2a5','#abdda4','#e6f598','#fee08b','#fdae61','#f46d43','#d53e4f'],
      autoMinMax: false, fixMin: 0, fixMax: 360 },
    ...
    { id: 'utci',
      label: 'UTCI',                        defaultChecked: false,
      getVal: function(e, ip) { return e.utci(); },
      unit: function(ip) { return '°C'; },
      steps: 9,
      colors: ['#313695','#4575b4','#74add1','#abd9e9','#e0f3f8','#ffffbf','#fee090','#fdae61','#f46d43','#d73027'],
      autoMinMax: false, fixMin: -40, fixMax: 46 },

    { id: 'utciCategoryIndex',
      label: 'UTCI Stress Category',              defaultChecked: false,
      getVal: function(e, ip) { var u = e.utci(); return u.map(function(v) { return _utciCategoryIndex(v); }); },
      unit: function(ip) { return ''; },
      fmt: _utciCatLabel,
      catLabels: _utciCatNames,
      steps: 9,
      // Rainbow: violet(0=extreme cold) → blue → cyan → green(6=comfort zone) → yellow → orange → red(10=extreme heat)
      colors: ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#4dcfb0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'],
      autoMinMax: false, fixMin: 0, fixMax: 10 }
    ];
    ...
    hmVarDefs.forEach(function(v) {
        if (!(v.id in window._hmSelectedVars)) {
            window._hmSelectedVars[v.id] = v.defaultChecked;
        }
    });
    ...
    function renderHeatmaps() {
        ...
        hmVarDefs.forEach(function(vd) {
            ...
            h3.innerHTML = vd.label + (unitLbl ? ' (' + unitLbl + ')' : '')
                + ' <button class="btn btn-outline-secondary btn-sm" style="vertical-align:middle; margin-left:8px;"'
                + ' onclick="downloadSVG(\'' + divId + '\',\'' + vd.id + '-heatmap\')">&#8659; SVG</button>'
                + ' <button class="btn btn-outline-secondary btn-sm" style="vertical-align:middle;"'
            ...
            var hmColors = (vd.catLabels && vd.colors) ? vd.colors : getHmColors();
            epwHeatmap(epwData(epw, cleanVals), {
                id:          '#' + divId,
                title:       vd.label + (unitLbl ? ' (' + unitLbl + ')' : ''),
                min_value:   minVal,
                max_value:   maxVal,
                unit:        unitLbl,
                ...
                fmtVal:      vd.fmt || null,
                catLabels:   vd.catLabels || null
            });

            // Set initial pct label
            d3.select('#' + divId).select('text.hm-pct-label')
    ...
    function applyHeatmapFilter(activeSet, n) {
        ...
        var pctText = n + ' of ' + totalHours + ' hours (' + pct + '%) selected';
        ...
        Object.keys(window._hmColorScales).forEach(function(divId) {
            ...
            d3.select('#' + divId).select('text.hm-pct-label').text(pctText);
        });
    }
...
function epwRadialChart(data,params) {
   ...
   //console.log(arc_data);
    ...
    // define svg size
    var margin = {top: 20, right: 220, bottom: 60, left: 20},
	...
	var _wrSvgW = width + margin.left + margin.right;
	var _wrSvgH = height + margin.top + margin.bottom;
	var svgEl = d3.select(params.id).append("svg")
		.attr("width", _wrSvgW)
		.attr("height", _wrSvgH);
	var svg = svgEl.append("g")
        .attr("transform", "translate(" + (cx + margin.left) + "," + (cy + margin.top) +")");

	// SVG-embedded chart title (if params.title provided)
	if (params.title) {
		svg.append('text')
			.attr('data-epv-title', '1')
			.attr('x', -cx - margin.left + 4)
			.attr('y', -cy - margin.top + 16)
	...
	var infoGroup = svgEl.append("g")
	  .attr("class","legend-header")
	  .attr("data-epv-datalabel","1")
	  .attr("transform", "translate(4," + (_wrSvgH - 28) + ")");
...
function epwHeatmap(data,params) {
	...
	var margin = {top: 26, right: params.catLabels ? 160 : 120, bottom: 40, left: 40},
	...
	// SVG-embedded pct label
	svg.append('text')
		.attr('class', 'hm-pct-label')
		.attr('data-epv-datalabel', '1')
	...
	if (params.catLabels) {
		// Categorical legend: one labeled swatch per category, bottom = category 0
		var nCat = params.catLabels.length;
		var swH  = height / nCat;
		legend.selectAll("rect.cat-swatch")
			.data(params.step_colors)
		  .enter().append("rect")
			.attr("class", "cat-swatch")
			.attr("x", 5)
			.attr("y", function(d, i) { return height - (i + 1) * swH; })
			.attr("width", 15)
			.attr("height", swH - 1)
			.style("fill", function(d) { return d; });
		legend.selectAll("text.cat-lbl")
			.data(params.catLabels)
		  .enter().append("text")
			.attr("class", "cat-lbl")
			.attr("x", 24)
			.attr("y", function(d, i) { return height - (i + 0.5) * swH; })
			.attr("dy", "0.35em")
			.style("font-size", "9px")
...
function epwValueFilter(epw) {
    ...
    // Solar altitude dimension (degrees, -20 to 90)
    var solAltDim = xfilter.dimension(function(d) { return d.solarAltitude === null || !isFinite(d.solarAltitude) ? -999 : d.solarAltitude; });
    var solAltArr = data.filter(function(d) { return d.solarAltitude !== null && isFinite(d.solarAltitude); }).map(function(d) { return d.solarAltitude; });
    var solAltMin = solAltArr.length ? Math.floor(Math.min.apply(Math, solAltArr)) : -20;
    var solAltMax = solAltArr.length ? Math.ceil(Math.max.apply(Math, solAltArr))  : 90;
    var solAltGroup = solAltDim.group(function(d) { return d < -90 ? solAltMin : Math.floor(d); });

    // Solar azimuth dimension (degrees, 0-360)
    var solAzmDim = xfilter.dimension(function(d) { return d.solarAzimuth === null || !isFinite(d.solarAzimuth) ? -999 : d.solarAzimuth; });
    var solAzmGroup = solAzmDim.group(function(d) { return d < 0 ? 0 : Math.floor(d / 2) * 2; });

    // UTCI stress category dimension (integer index 0–9; -1 for invalid hours)
    var utciCatLabels = ['Extreme Cold','Very Strong Cold','Strong Cold','Moderate Cold','Slight Cold','No Thermal Stress (cool)','Thermal Comfort Zone','Moderate Heat','Strong Heat','Very Strong Heat','Extreme Heat'];
    var utciCatColors = ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#4dcfb0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'];
    var utciCatDim   = xfilter.dimension(function(d) {
        return (d.utciCategoryIndex === null || d.utciCategoryIndex === undefined) ? -1 : d.utciCategoryIndex;
    });
    var utciCatGroup = utciCatDim.group(Math.round);

    // Numeric UTCI dimension for the bar-chart value filter
    var utciRawArr  = epw.utci();
    var _utciFinite = utciRawArr.filter(function(v) { return v !== null && isFinite(v); });
    var utciNumMin  = _utciFinite.length ? Math.floor(Math.min.apply(Math, _utciFinite) / 5) * 5 : -40;
    var utciNumMax  = _utciFinite.length ? Math.ceil( Math.max.apply(Math, _utciFinite) / 5) * 5 :  46;
    var utciNumDim   = xfilter.dimension(function(d) { return d.utci === null || !isFinite(d.utci) ? -999 : d.utci; });
    var utciNumGroup = utciNumDim.group(function(d) { return d < -900 ? -999 : Math.round(d); });
    ...
    d3.select("#solar-altitude-chart .title").text("Solar Altitude (\u00B0)");
    d3.select("#solar-azimuth-chart .title").text("Solar Azimuth (\u00B0)");
    d3.select("#utci-numeric-chart .title").text("UTCI (\u00B0C)");
    d3.select("#utci-chart .title").text("UTCI Stress Category");
    ...
    var charts = [
        ...
        epwBarChart()
            ...
            .group(enthalpys)
            .unit(unitSystem == 'IP' ? 'BTU/lb' : 'kJ/kg')
          .x(d3.scale.linear()
        ...
        epwBarChart()
            .dimension(utciNumDim)
            .group(utciNumGroup)
            .unit('\u00B0C')
          .x(d3.scale.linear()
            .domain([utciNumMin, utciNumMax])
            .rangeRound([0, 10 * 45])),

        epwHCatChart()
            .dimension(utciCatDim)
            .group(utciCatGroup)
            .labels(utciCatLabels)
            .colors(utciCatColors)
      ];

    var chart = d3.selectAll(".xfilterChart")
      .data(charts)
      .each(function(chart) { chart.on("brush", renderAllDebounced).on("brushend", renderAll); });
    ...
    // Debounced render used during brush drag — caps updates to ~80ms intervals.
    ...
    // Full render — compute filtered data once and share across all update hooks.
    function renderAll() {
      ...
      var filteredData = outputDim.top(Infinity);
      // Fast in-place updates run synchronously so they feel immediate
      if (window.updatePsychroPoints)  { window.updatePsychroPoints(filteredData); }
      if (window.updateWindRose)       { window.updateWindRose(filteredData); }
      if (window.updateHeatmaps)       { window.updateHeatmaps(filteredData); }
      if (window.updateTabular)        { window.updateTabular(filteredData); }
      if (window.updateScatter)        { window.updateScatter(filteredData); }
      // Heavy SVG-rebuild charts deferred so bar charts paint first
      var _fd = filteredData;
      setTimeout(function() {
        if (window.updateTimeseries)    { window.updateTimeseries(_fd); }
        if (window.updateDurationCurve) { window.updateDurationCurve(_fd); }
        if (window.updateDegreeDays)    { window.updateDegreeDays(_fd); }
        if (window.updateSunChart)      { window.updateSunChart(_fd); }
        if (window.updateOTCChart)      { window.updateOTCChart(_fd); }
      }, 0);
    }
    ...
    var vfVarList = [
        ...
        { divId: 'wetbulb-temperature-chart',  chartIdx: 5,  label: 'Wet Bulb Temperature',        defaultOn: false },
        ...
        { divId: 'solar-altitude-chart',              chartIdx: 26, label: 'Solar Altitude',               defaultOn: false },
        { divId: 'solar-azimuth-chart',               chartIdx: 27, label: 'Solar Azimuth',                defaultOn: false },
        { divId: 'utci-numeric-chart',                chartIdx: 28, label: 'UTCI',                          defaultOn: false },
        { divId: 'utci-chart',                        chartIdx: 29, label: 'UTCI Stress Category',           defaultOn: true  }
    ];
...
function epwBarChart() {
    var margin = {top: 28, right: 10, bottom: 20, left: 10},
        x,
        ...
        _inputMin = null,
        _inputMax = null,
        _g = null;   // persistent reference to chart <g> for label updates
    ...
    function chart(div) {
      ...
      div.each(function() {
        ...
        if (g.empty()) {
          // Read and remove the HTML title text node so the label lives only in
          // the SVG — this ensures it appears correctly in SVG/PNG exports.
          var titleNode = div.select('.title').node();
          var titleText = '';
          if (titleNode) {
              for (var ci = 0; ci < titleNode.childNodes.length; ci++) {
                  if (titleNode.childNodes[ci].nodeType === 3) {
                      titleText = titleNode.childNodes[ci].textContent.trim();
                      titleNode.removeChild(titleNode.childNodes[ci]);
                      break;
                  }
              }
          }
          ...
          // SVG-embedded title — data-epv-label marks it as togglable for export.
          g.append('text')
              .attr('class', 'epv-chart-title')
              .attr('data-epv-title', '1')
              .attr('x', 0)
              .attr('y', -14)
              .attr('text-anchor', 'start')
          ...
          // Active filter range text (updated live by brush drag and text inputs).
          g.append('text')
              .attr('class', 'epv-filter-range')
              .attr('data-epv-filter', '1')
              .attr('x', width)
              .attr('y', -14)
              .attr('text-anchor', 'end')
              .style('font-size', '9px')
        ...
        _g = g;   // keep closure reference current for brush/input event handlers
    ...
    brush.on("brush.chart", function() {
      ...
      g.select('.epv-filter-range').text(fmtVal(extent[0]) + ' \u2013 ' + fmtVal(extent[1]) + (unit ? '\u00A0' + unit : ''));
    });
    ...
    brush.on("brushend.chart", function() {
      if (brush.empty()) {
        ...
        d3.select(this.parentNode).select('.epv-filter-range').text('');
      }
    });
    ...
    chart.filter = function(_) {
      if (_) {
        ...
        if (_g) _g.select('.epv-filter-range').text(fmtVal(_[0]) + ' \u2013 ' + fmtVal(_[1]) + (unit ? '\u00A0' + unit : ''));
      } else {
        brush.clear();
        dimension.filterAll();
        if (_inputMin) { _inputMin.value = ''; _inputMax.value = ''; }
        if (_g) _g.select('.epv-filter-range').text('');
      }
      brushDirty = true;
      return chart;
    };
...
// Horizontal categorical bar chart for enumerated / label-indexed dimensions.
// Renders one horizontal bar per category.  Grey backing bar = total hours in that category
// at time of first render (before user filtering).  Coloured foreground = currently active
// (cross-filtered) count.  Click a bar or its label to toggle that category in / out of the
// crossfilter selection.  Multiple categories can be selected simultaneously.
function epwHCatChart() {
    var id       = window._epwChartIdCounter++,
        dimension,
        group,
        labels   = [],
        colors   = [],
        selectedKeys = null,   // null = no filter active;  Set<number> = selected category keys
        _bgData  = null,       // initial background totals, captured on first render
        dispatch = d3.dispatch("brush", "brushend");

    var margin = { top: 6, right: 16, bottom: 26, left: 148 };
    var barH   = 20;
    var barGap = 3;
    ...
    function chart(div) {
        div.each(function() {
            ...
            // Exclude the sentinel key -1 used for invalid / missing UTCI hours.
            var allData = group.all().filter(function(d) { return d.key >= 0; });

            // Capture background totals on first render, before any user interaction.
            if (!_bgData) {
                _bgData = allData.map(function(d) { return { key: d.key, value: d.value }; });
            }

            var nBars  = _bgData.length;
            var bgMax  = d3.max(_bgData, function(d) { return d.value; }) || 1;
            var innerW = 280;
            var innerH = nBars * (barH + barGap);
            var svgW   = margin.left + innerW + margin.right;
            var svgH   = margin.top  + innerH + margin.bottom;

            var xScale = d3.scale.linear().domain([0, bgMax]).range([0, innerW]);

            // Display order: highest key (hottest stress) at top, lowest key at bottom.
            var sortedKeys = _bgData.map(function(d) { return d.key; })
                                    .sort(function(a, b) { return b - a; });
            var keyRank = {};
            sortedKeys.forEach(function(k, i) { keyRank[k] = i; });
            function yPos(key) { return keyRank[key] * (barH + barGap); }

            // Build a fast lookup of current filtered counts by key.
            var fgByKey = {};
            allData.forEach(function(d) { fgByKey[d.key] = d.value; });

            // ---- One-time SVG + group setup ----
            var svg = container.select("svg.hcat-svg");
            if (svg.empty()) {
                var titleNode = container.select('.title').node();
                var titleText = '';
                if (titleNode) {
                    for (var ci = 0; ci < titleNode.childNodes.length; ci++) {
                        if (titleNode.childNodes[ci].nodeType === 3) {
                ...
                svg = container.append("svg").attr("class", "hcat-svg");
                var gRoot = svg.append("g").attr("class", "hcat-g")
                    .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
                gRoot.append("g").attr("class", "hcat-bg-bars");
                gRoot.append("g").attr("class", "hcat-fg-bars");
                gRoot.append("g").attr("class", "hcat-labels");
                gRoot.append("g").attr("class", "hcat-count-lbls");
                gRoot.append("g").attr("class", "hcat-xaxis axis");
            ...
            var g = svg.select(".hcat-g");

            // ---- Grey background bars (total hours) ----
            var bgBars = g.select(".hcat-bg-bars")
                .selectAll("rect.hcat-bg").data(_bgData, function(d) { return d.key; });
            bgBars.enter().append("rect").attr("class", "hcat-bg")
                .style("cursor", "pointer")
                .on("click", function(d) { toggleKey(d.key, container); });
            bgBars
                .attr("x", 0)
                .attr("y", function(d) { return yPos(d.key); })
                .attr("width",  function(d) { return xScale(d.value); })
                .attr("height", barH)
                .style("fill", "#d6d6d6");
            bgBars.exit().remove();

            // ---- Coloured foreground bars (filtered count) ----
            var fgBars = g.select(".hcat-fg-bars")
                .selectAll("rect.hcat-fg").data(_bgData, function(d) { return d.key; });
            fgBars.enter().append("rect").attr("class", "hcat-fg")
                .style("cursor", "pointer")
                .on("click", function(d) { toggleKey(d.key, container); });
            fgBars
                .attr("x", 0)
                .attr("y", function(d) { return yPos(d.key); })
                .attr("width",  function(d) { return xScale(fgByKey[d.key] || 0); })
                .attr("height", barH)
                .style("fill", function(d) {
                    if (selectedKeys && !selectedKeys.has(d.key)) return "#bbb";
                    return colors[d.key] || (window._plotColor || 'steelblue');
                })
                .style("fill-opacity", function(d) {
                    if (selectedKeys && !selectedKeys.has(d.key)) return 0.35;
                    return 1;
                });
            fgBars.exit().remove();

            // ---- Y-axis category labels ----
            var lbls = g.select(".hcat-labels")
                .selectAll("text.hcat-lbl").data(_bgData, function(d) { return d.key; });
            lbls.enter().append("text").attr("class", "hcat-lbl")
                .style("cursor", "pointer")
                .on("click", function(d) { toggleKey(d.key, container); });
            lbls
                .attr("x", -6)
                .attr("y", function(d) { return yPos(d.key) + barH / 2; })
                .attr("dy", "0.35em")
                .attr("text-anchor", "end")
                .style("font-size", "11px")
                .style("fill", function(d) {
                    if (selectedKeys && !selectedKeys.has(d.key)) return "#aaa";
                    return "#333";
                })
                .text(function(d) {
                    return labels[d.key] !== undefined ? labels[d.key] : String(d.key);
                });
            lbls.exit().remove();

            // ---- Per-bar count + percent data labels ----
            // Place label inside the bar if it fits, otherwise just outside the bar end.
            var bgTotal = d3.sum(_bgData, function(d) { return d.value; }) || 1;
            var CHAR_W  = 6.5;   // approximate px per character at 10px font
            var PAD     = 4;     // padding inside/outside the bar end
            var cntLbls = g.select('.hcat-count-lbls')
                .selectAll('text.hcat-cnt').data(_bgData, function(d) { return d.key; });
            cntLbls.enter().append('text').attr('class', 'hcat-cnt')
                .attr('data-epv-datalabel', '1');
            cntLbls
                .attr('y', function(d) { return yPos(d.key) + barH / 2; })
                .attr('dy', '0.35em')
                ...
                .each(function(d) {
                    var n      = fgByKey[d.key] || 0;
                    var pct    = Math.round(n / bgTotal * 100);
                    var label  = n + ' (' + pct + '%)';
                    var barPx  = xScale(n);
                    var lblPx  = label.length * CHAR_W + PAD * 2;
                    var inside = barPx >= lblPx;
                    d3.select(this)
                        .attr('x',            inside ? barPx - PAD : barPx + PAD)
                        .attr('text-anchor',  inside ? 'end'        : 'start')
                        .style('fill',        inside ? '#fff'       : '#555')
                        .text(label);
                });
            cntLbls.exit().remove();
            ...
            container.select(".title a.reset").style("display", selectedKeys ? null : "none");
        });
    }

    function toggleKey(key, container) {
        if (!selectedKeys) {
            selectedKeys = new Set([key]);
        } else if (selectedKeys.has(key)) {
            selectedKeys.delete(key);
            if (selectedKeys.size === 0) selectedKeys = null;
        } else {
    ...
    function applyDimFilter() {
        if (selectedKeys === null) {
            ...
            dimension.filterFunction(function(d) { return keys.has(d); });
        }
    }
    ...
    chart.filter = function(_) {
        if (_ === null || _ === undefined) {
            selectedKeys = null;
            if (dimension) dimension.filterAll();
        }
        return chart;
    };
...
function epwPolarChart() {
    ...
    function chart(div) {
        div.each(function() {
            ...
            var titleTopPad = 22;  // extra px at top for SVG-embedded title
            var svgW = (outerRadius + 22) * 2;
            var svgH = (outerRadius + 22) * 2 + titleTopPad;
            var cx = svgW / 2, cy = svgH / 2 + titleTopPad / 2;
            ...
            if (svg.empty()) {
                ...
                // SVG-embedded title — sits at top-left above the polar circle
                svg.append('text')
                    .attr('class', 'epv-chart-title')
                    .attr('data-epv-title', '1')
                    .attr('x', 4)
    ...
    function drawBars(g, n, groupData, rScale, angleStep) {
        ...
        fgGroup.selectAll("path.pfg")
            ...
            .style("fill-opacity", function(d, i) {
                if (!filterExtent) return 1;
                var s = filterExtent[0], e = filterExtent[1];
                var active = (s <= e) ? (i >= s && i <= e) : (i >= s || i <= e);
                return active ? 1 : 0;
            })
            .style("stroke", manySegs ? "none" : "white")
            .style("stroke-width", manySegs ? "0" : "0.5px");
...
function epwPsychroChart(epw) {
    ...
    // Chart geometry
    var margin = {top: 36, right: 130, bottom: 50, left: 70};
    var width  = 720;
    var height = 450;
    ...
    var _psSvgW = width  + margin.left + margin.right;
    var _psSvgH = height + margin.top  + margin.bottom;
    var _psSvgEl = d3.select("#epwPsychroChart").append("svg")
        .attr("width",  _psSvgW)
        .attr("height", _psSvgH);
    var svg = _psSvgEl.append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    // SVG-embedded chart title
    svg.append('text')
        .attr('data-epv-title', '1')
        .attr('x', 0).attr('y', -22)
        .attr('text-anchor', 'start')
    ...
    var legendGroup = svg.append("g").attr("class", "psychro-legend").attr("data-epv-legend", "1");
    ...
    var hoursLabel = _psSvgEl.append("text")
        .attr("class", "legend-header")
        .attr("data-epv-datalabel", "1")
        .attr("x", 4)
        .attr("y", _psSvgH - 6)
...
function epwScatterChart(epw) {
    ...
    function populateSelect(elId, selectedId) {
        ...
        varDefs.forEach(function(v) {
            ...
            opt.value = v.id;
            opt.textContent = v.label + (v.unit ? ' (' + v.unit + ')' : '');
    ...
    // Chart geometry — left margin adjusts dynamically in redrawAxes based on y variable type
    var margin = { top: 34, right: 30, bottom: 60, left: 90 };
    var width  = 720;
    var height = 430;

    var _scSvgW = width  + margin.left + margin.right;
    var _scSvgH = height + margin.top  + margin.bottom;
    var _scSvgEl = d3.select('#epwScatterChart').append('svg')
        .attr('width',  _scSvgW)
        .attr('height', _scSvgH);
    var svg = _scSvgEl.append('g')
        .attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    var titleEl  = svg.append('text').attr('data-epv-title', '1')
                      .attr('x', 0).attr('y', -20)
                      .attr('text-anchor', 'start')
                      .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
                      .text('Scatter Plot');
    var hoursLabel = _scSvgEl.append('text').attr('class', 'legend-header')
                       .attr('data-epv-datalabel', '1')
                       .attr('x', 4).attr('y', _scSvgH - 6)
                       .style('font-size', '12px').style('fill', '#555')
                       .text(totalHours + ' of ' + totalHours + ' hours (100%) selected');
    ...
    function redrawAxes() {
        ...
        // Adjust left margin dynamically: wide for categorical y-axis labels, narrower otherwise
        var newLeft = yVd.catLabels ? 130 : 70;
        if (newLeft !== margin.left) {
            margin.left = newLeft;
            _scSvgEl.attr('width', width + margin.left + margin.right);
            svg.attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
            yLabelEl.attr('y', -(margin.left - 15));
        }
        ...
        if (xVd.catLabels) {
            x = d3.scale.linear().domain([xMin - 0.5, xMax + 0.5]).range([0, width]);
            xAxisG.call(d3.svg.axis().scale(x).orient('bottom')
                .tickValues(d3.range(Math.round(xMin), Math.round(xMax) + 1))
                .tickFormat(function(v) { return xVd.fmt ? xVd.fmt(Math.round(v)) : String(v); }));
            _wrapCatXAxisLabels(xAxisG);
        } else {
            x = d3.scale.linear().domain([xMin - xPad, xMax + xPad]).range([0, width]);
            xAxisG.call(d3.svg.axis().scale(x).orient('bottom').ticks(8).tickFormat(function(v) { return xVd.fmt ? xVd.fmt(v) : v; }));
        }

        y = d3.scale.linear().domain([yMin - yPad, yMax + yPad]).range([height, 0]);

        var _scYAxisCall = yVd.catLabels
            ? d3.svg.axis().scale(y).orient('left')
                .tickValues(d3.range(Math.round(yMin), Math.round(yMax) + 1))
                .tickFormat(function(v) { return yVd.fmt ? yVd.fmt(Math.round(v)) : String(v); })
            : d3.svg.axis().scale(y).orient('left').ticks(8).tickFormat(function(v) { return yVd.fmt ? yVd.fmt(v) : v; });
        var _scYAxisApplied = yAxisG.call(_scYAxisCall);
        if (yVd.catLabels) { _singleLineCatAxisLabels(_scYAxisApplied); }
        xLabelEl.text(xVd.label + (xVd.unit ? ' (' + xVd.unit + ')' : ''));
        yLabelEl.text(yVd.label + (yVd.unit ? ' (' + yVd.unit + ')' : ''));
        if (titleEl) titleEl.text(xVd.label + ' vs. ' + yVd.label);
    }
...
function epwTimeseriesCharts(epw) {
    ...
    function drawAnnualTimeseries(activeData, varDef) {
        ...
        var allStats    = _cachedAllStatsDailyAnn || computeDailyStats(allData, varDef);
        ...
        var margin  = { top: 34, right: 20, bottom: 40, left: varDef.catLabels ? 144 : 62 };
        var pxPerDay = 2.5;
        ...
        // SVG-embedded title and legend
        var _tsColor = window._plotColor || 'steelblue';
        svg.append('text')
            .attr('data-epv-title', '1')
            .attr('x', margin.left).attr('y', 16)
            .attr('text-anchor', 'start')
            .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
            .text(varDef.label + ' \u2014 Annual');
        var _lgd1 = svg.append('g')
            .attr('data-epv-legend', '1')
            .attr('transform', 'translate(' + (totalW - margin.right - 140) + ',6)');
        _lgd1.append('rect').attr('width', 18).attr('height', 8).attr('y', 3).style('fill', '#ddd').style('fill-opacity', 0.9);
        _lgd1.append('text').attr('x', 22).attr('y', 10).style('font-size', '10px').attr('fill', '#666').text('All data (min\u2013max + avg)');
        _lgd1.append('rect').attr('width', 18).attr('height', 8).attr('y', 17).style('fill', _tsColor).style('fill-opacity', 0.35);
        _lgd1.append('text').attr('x', 22).attr('y', 24).style('font-size', '10px').attr('fill', '#666').text('Filtered (min\u2013max + avg)');

        // % selected label
        var _ts1Pct = allData.length > 0 ? (activeData.length / allData.length * 100).toFixed(1) : '0.0';
        svg.append('text')
            .attr('data-epv-datalabel', '1')
            .attr('x', 4).attr('y', totalH - 4)
            .attr('text-anchor', 'start')
            .style('font-size', '12px').style('fill', '#555')
            .text(activeData.length + ' of ' + allData.length + ' hours (' + _ts1Pct + '%) selected');
        ...
        // ---- Y Axis ----
        var _tsCatTicks = varDef.catLabels ? (function() {
            var vals = allData.map(function(d) { return d[varDef.id]; }).filter(function(v) { return v !== null && isFinite(v); });
            var lo = Math.round(d3.min(vals)), hi = Math.round(d3.max(vals));
            return d3.range(lo, hi + 1);
        }()) : null;
        var yAxis = varDef.catLabels
            ? d3.svg.axis().scale(yScale).orient('left')
                .tickValues(_tsCatTicks)
                .tickFormat(function(v) { return varDef.fmt ? varDef.fmt(Math.round(v)) : String(Math.round(v)); })
            : d3.svg.axis().scale(yScale).orient('left').ticks(6);
        var _tsYAxisG = g.append('g').attr('class', 'axis').call(yAxis);
        if (varDef.catLabels) { _singleLineCatAxisLabels(_tsYAxisG); }

        g.append('text')
            .attr('transform', 'rotate(-90)')
            .attr('x', -innerH / 2).attr('y', -margin.left + 17).attr('dy', '1em')
            .style('text-anchor', 'middle').style('font-size', '13px')
            .text(varDef.label + (varDef.unit ? ' (' + varDef.unit + ')' : ''));
    ...
    function drawDailyProfiles(activeData, varDef) {
        ...
        var allStats    = _cachedAllStatsProfile || computeHourlyProfileByMonth(allData, varDef);
        ...
        var panelInnerH  = 170;
        var panelGutter  = 4;
        var marginLeft   = varDef.catLabels ? 130 : 56;   // space for shared y-axis
        var marginTop    = 36;   // space for month name labels
        var marginBottom = 46;   // space for x-axis ticks + % selected label
        var marginRight  = 8;
        ...
        // SVG-embedded title and legend
        var _tsColor2 = window._plotColor || 'steelblue';
        svg.append('text')
            .attr('data-epv-title', '1')
            .attr('x', marginLeft).attr('y', 16)
            .attr('text-anchor', 'start')
            .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
            .text(varDef.label + ' — Daily Profiles by Month');
        var _lgd2 = svg.append('g')
            .attr('data-epv-legend', '1')
            .attr('transform', 'translate(' + (totalW - 300) + ',4)');
        _lgd2.append('rect').attr('width', 14).attr('height', 10).attr('y', 2).style('fill', '#d8d8d8').style('fill-opacity', 0.9);
        _lgd2.append('text').attr('x', 17).attr('y', 11).style('font-size', '10px').attr('fill', '#666').text('All data (min–max + avg)');
        _lgd2.append('rect').attr('x', 145).attr('width', 14).attr('height', 10).attr('y', 2).style('fill', _tsColor2).style('fill-opacity', 0.35);
        _lgd2.append('text').attr('x', 162).attr('y', 11).style('font-size', '10px').attr('fill', '#666').text('Filtered (min–max + avg)');

        // % selected label
        var _ts2Pct = allData.length > 0 ? (activeData.length / allData.length * 100).toFixed(1) : '0.0';
        svg.append('text')
            .attr('data-epv-datalabel', '1')
            .attr('x', 4).attr('y', totalH - 4)
            .attr('text-anchor', 'start')
            .style('font-size', '12px').style('fill', '#555')
            .text(activeData.length + ' of ' + allData.length + ' hours (' + _ts2Pct + '%) selected');
        ...
        var _dpCatTicks = varDef.catLabels ? (function() {
            var vals = allData.map(function(d) { return d[varDef.id]; }).filter(function(v) { return v !== null && isFinite(v); });
            var lo = Math.round(d3.min(vals)), hi = Math.round(d3.max(vals));
            return d3.range(lo, hi + 1);
        }()) : null;
        var yAxis = varDef.catLabels
            ? d3.svg.axis().scale(yScale).orient('left')
                .tickValues(_dpCatTicks)
                .tickFormat(function(v) { return varDef.fmt ? varDef.fmt(Math.round(v)) : String(Math.round(v)); })
            : d3.svg.axis().scale(yScale).orient('left').ticks(5);
        var _dpYAxisG = svg.append('g')
            .attr('class', 'axis')
            .attr('transform', 'translate(' + marginLeft + ',' + marginTop + ')')
            .call(yAxis);
        if (varDef.catLabels) { _singleLineCatAxisLabels(_dpYAxisG); }
    ...
    // Cache all-data stats per variable — only recomputed on variable change, not on filter changes.
    var _cachedAllStatsVarId   = null;
    var _cachedAllStatsDailyAnn = null;
    var _cachedAllStatsProfile  = null;

    function redrawAll(activeData) {
        var varDef = getVarDef(window._tsCurrentVarId);
        // Recompute all-data stats only when the selected variable changes
        if (_cachedAllStatsVarId !== varDef.id) {
            _cachedAllStatsVarId   = varDef.id;
            _cachedAllStatsDailyAnn = computeDailyStats(allData, varDef);
            _cachedAllStatsProfile  = computeHourlyProfileByMonth(allData, varDef);
        }
...
function epwDurationCurve(epw) {
  ...
  // Populate variable select; rebuild each render so units stay in sync
  ...
  // Cache sorted full-data per variable (sort is expensive and never changes between filter updates)
  var _durSortedCache = {};
  function getAllSorted(varId) {
    if (!_durSortedCache[varId]) {
      _durSortedCache[varId] = allData.slice().sort(function(a, b) { return b[varId] - a[varId]; });
    }
    return _durSortedCache[varId];
  }
  ...
  function draw(activeData) {
    ...
    var margin = { top: 24, right: vDef.catLabels ? 220 : 165, bottom: 60, left: vDef.catLabels ? 110 : 70 };
    var svgW = 880, svgH = 464;
    var W = svgW - margin.left - margin.right;
    var H = svgH - margin.top  - margin.bottom;

    var allSorted = getAllSorted(varId);
    ...
    var durSvgEl = container.append('svg')
        .attr('width', svgW)
        .attr('height', svgH);
    var svg = durSvgEl.append('g')
        .attr('transform', 'translate(' + margin.left + ',' + margin.top + ')');
    ...
    svg.append('text')
        ...
        .attr('text-anchor', 'start')
        .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
        .text(vDef.label + (getUnit(vDef) ? ' (' + getUnit(vDef) + ')' : '') + ' — Duration Curve');

    // Axes
    var xAxis = d3.svg.axis().scale(xScale).orient('bottom').ticks(8);
    var _durCatTicks = vDef.catLabels ? (function() {
        var vals = allData.map(function(d) { return d[vDef.id]; }).filter(function(v) { return v !== null && isFinite(v); });
        var lo = Math.round(d3.min(vals)), hi = Math.round(d3.max(vals));
        return d3.range(lo, hi + 1);
    }()) : null;
    var yAxis = vDef.catLabels
        ? d3.svg.axis().scale(yScale).orient('left')
            .tickValues(_durCatTicks)
            .tickFormat(function(v) { return vDef.fmt ? vDef.fmt(Math.round(v)) : String(Math.round(v)); })
        : d3.svg.axis().scale(yScale).orient('left').ticks(8)
            .tickFormat(function(v) { return vDef.fmt ? vDef.fmt(v) : v.toFixed(1); });
    ...
    var _durYAxisG = svg.append('g')
        .attr('class', 'y axis')
        .call(yAxis);
    if (vDef.catLabels) { _singleLineCatAxisLabels(_durYAxisG); }
    ...
    // % selected label
    var durPct = allN > 0 ? (N / allN * 100).toFixed(1) : '0.0';
    durSvgEl.append('text')
        .attr('class', 'dur-pct-label')
        .attr('data-epv-datalabel', '1')
        .attr('x', 4).attr('y', svgH - 6)
        .attr('text-anchor', 'start')
        .style('font-size', '12px').style('fill', '#555')
        .text(N + ' of ' + allN + ' hours (' + durPct + '%) selected');
  }
  ...
  // Allow PV recalculate to bust the sort cache for pvAC
  window._durClearSortCache = function() { _durSortedCache = {}; };
}
...
function epwDegreeDays(epw) {
  ...
  function draw(activeData) {
    ...
    var ddHrsPct = allData.length > 0 ? (activeData.length / allData.length * 100).toFixed(1) : '0.0';
    document.getElementById('ddTotals').innerHTML =
      ...
      '&nbsp;&nbsp;&nbsp;&nbsp;' +
      '<span style="color:#555;">' + activeData.length + ' of ' + allData.length + ' hours (' + ddHrsPct + '%) selected</span>';
    ...
    monthNames.forEach(function(mName, mi) {
      ...
      [
        ...
        if (bar.filtVal > 0) {
          ...
          // Per-bar data label
          var barTotal = bar.key === 'hdd' ? totalHDD : totalCDD;
          var barPct = barTotal > 0 ? (bar.filtVal / barTotal * 100).toFixed(0) : '0';
          var barLbl = svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw / 2)
              .attr('y', yScale(bar.filtVal) - 15)
              .attr('text-anchor', 'middle')
              .style('font-size', '10px')
              .style('fill', bar.colorFilt);
          barLbl.append('tspan').attr('x', bx + bw / 2).attr('dy', 0).text(Math.round(bar.filtVal));
          barLbl.append('tspan').attr('x', bx + bw / 2).attr('dy', '1.2em').text('(' + barPct + '%)');
        }
      });
    });
    ...
    svg.append('text')
        ...
        .attr('text-anchor', 'middle')
        .style('font-size', '12px').style('fill', '#555')
        .text('HDD(' + hBase + uLabel + '): ' + Math.round(totalHDD) + '  |  CDD(' + cBase + uLabel + '): ' + Math.round(totalCDD) + '  |  ' + activeData.length + ' of ' + allData.length + ' hours (' + ddHrsPct + '%) selected');

    // SVG-embedded legend (4 swatches in a horizontal row, inline with title)
    var ddLegG = svg.append('g')
        .attr('data-epv-legend', '1')
        .attr('transform', 'translate(' + (W - 358) + ',-17)');
    [
      { color: HDD_FILT, label: 'HDD (filtered)', x: 0   },
      { color: HDD_ALL,  label: 'HDD (all data)', x: 88  },
      { color: CDD_FILT, label: 'CDD (filtered)', x: 176  },
      { color: CDD_ALL,  label: 'CDD (all data)', x: 264  }
    ].forEach(function(item) {
      ddLegG.append('rect').attr('x', item.x).attr('y', 0).attr('width', 14).attr('height', 10).attr('fill', item.color);
      ddLegG.append('text').attr('x', item.x + 17).attr('y', 9).style('font-size', '9px').attr('fill', '#333').text(item.label);
    });
  }

  // ---- By-temperature chart ----
  function drawByTemp(activeData) {
    var container = d3.select('#epwDegreeDaysByTempChart');
    container.select('svg').remove();

    var hBase   = window._ddHeatBase || defaultBase;
    var cBase   = window._ddCoolBase || defaultBase;
    var uLabel  = isIP ? '\u00b0F' : '\u00b0C';
    var binSize = isIP ? 5 : 2;

    // Determine bin range from all-data dry-bulb temperatures
    var allTemps = allData.map(function(d) { return d.dryBulbTemperature; });
    var tMin = Math.floor(d3.min(allTemps) / binSize) * binSize;
    var tMax = Math.ceil(d3.max(allTemps)  / binSize) * binSize;

    var bins = [];
    for (var t = tMin; t < tMax; t += binSize) { bins.push(t); }

    function accumBins(data) {
      var hdd = {}, cdd = {};
      bins.forEach(function(b) { hdd[b] = 0; cdd[b] = 0; });
      data.forEach(function(d) {
        var db  = d.dryBulbTemperature;
        var bin = Math.floor(db / binSize) * binSize;
        if (hdd[bin] === undefined) return;
        hdd[bin] += Math.max(0, hBase - db) / 24;
        cdd[bin] += Math.max(0, db - cBase) / 24;
      });
      return { hdd: hdd, cdd: cdd };
    }

    var allBins    = accumBins(allData);
    var activeBins = accumBins(activeData);

    // Compute totals for SVG label
    var totalHDD2 = d3.sum(bins, function(b) { return activeBins.hdd[b] || 0; });
    var totalCDD2 = d3.sum(bins, function(b) { return activeBins.cdd[b] || 0; });
    var ddHrsPct2 = allData.length > 0 ? (activeData.length / allData.length * 100).toFixed(1) : '0.0';

    var margin2 = { top: 50, right: 30, bottom: 80, left: 75 };
    var svg2W = 920, svg2H = 310;
    var W2 = svg2W - margin2.left - margin2.right;
    var H2 = svg2H - margin2.top  - margin2.bottom;

    var xScale = d3.scale.ordinal().domain(bins).rangeRoundBands([0, W2], 0.1);

    var yMax = d3.max(bins, function(b) {
      return Math.max(allBins.hdd[b] || 0, allBins.cdd[b] || 0);
    }) || 1;
    var yScale = d3.scale.linear().domain([0, yMax * 1.06]).range([H2, 0]);

    var svgEl2 = container.append('svg').attr('width', svg2W).attr('height', svg2H);
    var svg = svgEl2.append('g')
    ...
    svg.append('text')
        ...
        .attr('text-anchor', 'start')
        .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
        .text('Degree Days by Temperature Bin (' + binSize + uLabel + ' bins)');

    var bw = Math.max(1, Math.floor(xScale.rangeBand() / 2) - 1);

    bins.forEach(function(b) {
      var bx      = xScale(b);
      var hddAll  = allBins.hdd[b]    || 0;
      var cddAll  = allBins.cdd[b]    || 0;
      var hddFilt = activeBins.hdd[b] || 0;
      var cddFilt = activeBins.cdd[b] || 0;

      if (hddAll > 0) {
        svg.append('rect').attr('x', bx).attr('y', yScale(hddAll))
            .attr('width', bw).attr('height', H2 - yScale(hddAll)).attr('fill', HDD_ALL);
      }
      if (hddFilt > 0) {
        svg.append('rect').attr('x', bx).attr('y', yScale(hddFilt))
            .attr('width', bw).attr('height', H2 - yScale(hddFilt)).attr('fill', HDD_FILT)
            .on('mouseover', function() { tooltip.style('display', 'block'); })
            .on('mousemove', (function(b2, v) {
              return function() {
                tooltip.style('left', (d3.event.clientX + 14) + 'px')
                       .style('top',  (d3.event.clientY - 28) + 'px')
                       .html('<b>HDD ' + b2 + '\u2013' + (b2 + binSize) + uLabel + '</b>: ' + v.toFixed(2) + ' degree-days');
              };
            }(b, hddFilt)))
            .on('mouseout', function() { tooltip.style('display', 'none'); });
        if (Math.round(hddFilt) > 0) {
          var hddPct = totalHDD2 > 0 ? (hddFilt / totalHDD2 * 100).toFixed(0) : '0';
          var hddLbl = svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw / 2).attr('y', yScale(hddFilt) - 14)
              .attr('text-anchor', 'middle')
              .style('font-size', '9px').style('fill', HDD_FILT);
          hddLbl.append('tspan').attr('x', bx + bw / 2).attr('dy', 0).text(Math.round(hddFilt));
          hddLbl.append('tspan').attr('x', bx + bw / 2).attr('dy', '1.2em').text('(' + hddPct + '%)');
        }
      }

      if (cddAll > 0) {
        svg.append('rect').attr('x', bx + bw + 1).attr('y', yScale(cddAll))
            .attr('width', bw).attr('height', H2 - yScale(cddAll)).attr('fill', CDD_ALL);
      }
      if (cddFilt > 0) {
        svg.append('rect').attr('x', bx + bw + 1).attr('y', yScale(cddFilt))
            .attr('width', bw).attr('height', H2 - yScale(cddFilt)).attr('fill', CDD_FILT)
            .on('mouseover', function() { tooltip.style('display', 'block'); })
            .on('mousemove', (function(b2, v) {
              return function() {
                tooltip.style('left', (d3.event.clientX + 14) + 'px')
                       .style('top',  (d3.event.clientY - 28) + 'px')
                       .html('<b>CDD ' + b2 + '\u2013' + (b2 + binSize) + uLabel + '</b>: ' + v.toFixed(2) + ' degree-days');
              };
            }(b, cddFilt)))
            .on('mouseout', function() { tooltip.style('display', 'none'); });
        if (Math.round(cddFilt) > 0) {
          var cddPct = totalCDD2 > 0 ? (cddFilt / totalCDD2 * 100).toFixed(0) : '0';
          var cddLbl = svg.append('text')
              .attr('data-epv-datalabel', '1')
              .attr('x', bx + bw + 1 + bw / 2).attr('y', yScale(cddFilt) - 14)
              .attr('text-anchor', 'middle')
              .style('font-size', '9px').style('fill', CDD_FILT);
          cddLbl.append('tspan').attr('x', bx + bw + 1 + bw / 2).attr('dy', 0).text(Math.round(cddFilt));
          cddLbl.append('tspan').attr('x', bx + bw + 1 + bw / 2).attr('dy', '1.2em').text('(' + cddPct + '%)');
        }
      }
    });

    // X-axis: all bins labeled with range + units, rotated 45° to prevent overlap
    var xAxis = d3.svg.axis().scale(xScale).orient('bottom')
        .tickFormat(function(b) { return b + '\u2013' + (b + binSize) + uLabel; });
    svg.append('g').attr('class', 'x axis')
        .attr('transform', 'translate(0,' + H2 + ')').call(xAxis)
      .selectAll('text')
    ...
    svg.append('g').attr('class', 'y axis')
      ...
      .append('text')
        .attr('transform', 'rotate(-90)').attr('x', -H2 / 2).attr('y', -60)
        .attr('fill', '#333').style('font-size', '13px').style('text-anchor', 'middle')
        .text('Degree-Days');

    // SVG totals label
    svg.append('text')
        .attr('data-epv-datalabel', '1')
        .attr('x', W2 / 2).attr('y', H2 + 56)
        .attr('text-anchor', 'middle')
        .style('font-size', '12px').style('fill', '#555')
        .text('HDD(' + hBase + uLabel + '): ' + Math.round(totalHDD2) + '  |  CDD(' + cBase + uLabel + '): ' + Math.round(totalCDD2) + '  |  ' + activeData.length + ' of ' + allData.length + ' hours (' + ddHrsPct2 + '%) selected');

    // Dashed reference lines at each base temperature
    [
      { base: hBase, color: HDD_FILT, label: 'Heat base ' + hBase + uLabel, anchor: 'end',   dx: -3, dy: 12 },
      { base: cBase, color: CDD_FILT, label: 'Cool base ' + cBase + uLabel, anchor: 'start', dx:  3, dy: 24 }
    ].forEach(function(ref) {
      var refBin = Math.floor(ref.base / binSize) * binSize;
      var refX = xScale(refBin);
      if (refX === undefined) return;
      svg.append('line')
          .attr('x1', refX).attr('x2', refX).attr('y1', 0).attr('y2', H2)
          .attr('stroke', ref.color).attr('stroke-width', 1.5)
          .attr('stroke-dasharray', '4,3').attr('opacity', 0.8);
      svg.append('text')
          .attr('x', refX + ref.dx).attr('y', ref.dy)
          .attr('text-anchor', ref.anchor)
          .style('font-size', '9px').attr('fill', ref.color).text(ref.label);
    });

    // Legend (inline with title, identical style/spacing to monthly chart)
    var ddLegG2 = svg.append('g')
        .attr('data-epv-legend', '1')
        .attr('transform', 'translate(' + (W2 - 358) + ',-41)');
    [
      { color: HDD_FILT, label: 'HDD (filtered)', x: 0   },
      { color: HDD_ALL,  label: 'HDD (all data)', x: 88  },
      { color: CDD_FILT, label: 'CDD (filtered)', x: 176  },
      ...
      ddLegG2.append('rect').attr('x', item.x).attr('y', 0).attr('width', 14).attr('height', 10).attr('fill', item.color);
      ddLegG2.append('text').attr('x', item.x + 17).attr('y', 9).style('font-size', '9px').attr('fill', '#333').text(item.label);
    });
  }

  function drawBoth(activeData) {
    draw(activeData);
    drawByTemp(activeData);
  }

  drawBoth(window._ddLastActive || allData);
...
function epwSolarPV(epw) {
  ...
  if (btn) {
    ...
    btn.onclick = function () {
      draw();
      // Clear duration curve sort cache so pvAC resorting uses fresh values
      if (window._durClearSortCache) window._durClearSortCache();
...
/*-------------------------------------------------------------------------
 * epwSunChart
 * Stereographic (orthographic-projection) sun path chart.
 * Shows the analemma (figure-eight) loop for each hour of the day across
 * all days of the year.  Points are coloured by the selected variable.
 *-------------------------------------------------------------------------*/
function epwSunChart(epw) {
  var isIP = (unitSystem === 'IP');

  // ---- Variable definitions (matches timeseries full list) ----
  var varDefs = epwVarDefs(isIP);
  varDefs.unshift({ id: 'none', label: 'None (sun position only)', unitIP: '', unitSI: '', unit: '', isSolar: false });

  // Populate dropdown (only once; clone to strip stale listeners on re-render)
  var sel = document.getElementById('scVarSelect');
  if (!sel) return;
  var selClone = sel.cloneNode(false);
  sel.parentNode.replaceChild(selClone, sel);
  sel = selClone;
  ...
  if (!window._sunVarId) window._sunVarId = 'none';
  sel.value = window._sunVarId;

  sel.addEventListener('change', function() {
    window._sunVarId = this.value;
    // Reset last-var trackers so the next filter update triggers a full rebuild
    _sunLastVarId     = this.value;
    _sunCartLastVarId = this.value;
    drawSunChart(window._sunLastActive || null);
    drawCartesianSunChart(window._sunLastActive || null);
  });

  // ---- Build enriched data array (daytime only) ----
  var allTsData = window._tsAllData;
  if (!allTsData || !allTsData.length) return;

  var solPos = getSolarPositions(epw);
  if (!solPos) return;

  // Pre-build daytime lookup (altitude > 0°)
  var allDaytime = [];
  for (var ix = 0; ix < allTsData.length; ix++) {
    var alt = solPos.altitude[ix];
    if (alt === null || !isFinite(alt) || alt <= 0) continue;
    var d = allTsData[ix];
    allDaytime.push({
      index:   ix,
      month:   d.month,
      ...
      hour:    d.hour,
      alt:     alt,
      azm:     solPos.azimuth[ix],
      // pre-read variable values
      dryBulbTemperature:        d.dryBulbTemperature,
      dewPointTemperature:       d.dewPointTemperature,
      wetBulbTemperature:        d.wetBulbTemperature,
      relativeHumidity:          d.relativeHumidity,
      humidityRatio:             d.humidityRatio,
      enthalpy:                  d.enthalpy,
      globalHorizontalRadiation: d.globalHorizontalRadiation,
      directNormalRadiation:     d.directNormalRadiation,
      diffuseHorizontalRadiation:d.diffuseHorizontalRadiation,
      windSpeed:                 d.windSpeed,
      windDirection:             d.windDirection,
      totalSkyCover:             d.totalSkyCover,
      opaqueSkyCover:            d.opaqueSkyCover,
      visibility:                d.visibility,
      ceilingHeight:             d.ceilingHeight,
      precipitableWater:         d.precipitableWater,
      aerosolOpticalDepth:       d.aerosolOpticalDepth,
      snowDepth:                 d.snowDepth,
      daysSinceLastSnowfall:     d.daysSinceLastSnowfall,
      albedo:                    d.albedo,
      liquidPrecipitationDepth:  d.liquidPrecipitationDepth,
      liquidPrecipitationQuantity: d.liquidPrecipitationQuantity,
      pressure:                  d.pressure,
      pvAC:                      d.pvAC,
      utci:                      d.utci,
      utciCategoryIndex:         d.utciCategoryIndex
    });
  }
  ...
  // ---- Drawing ----
  function drawSunChart(activeSet) {
    var container = d3.select('#epwSunChart');
    container.select('svg').remove();

    var varId  = window._sunVarId || 'none';
    var vDef   = varDefs.filter(function(v) { return v.id === varId; })[0] || varDefs[0];
    var color  = window._plotColor || 'steelblue';

    // ---- SVG geometry ----
    var margin = { top: 50, right: 220, bottom: 40, left: 60 };
    var svgW = 720, svgH = 560;
    var W = svgW - margin.left - margin.right;  // drawing area width
    var R = Math.min(W, svgH - margin.top - margin.bottom) / 2;  // chart radius

    var svgEl = container.append('svg')
        .attr('width', svgW)
        .attr('height', svgH);
    var svg = svgEl.append('g')
        .attr('transform', 'translate(' + (margin.left + W / 2) + ',' + (margin.top + R + 10) + ')');

    // SVG-embedded title (placed in SVG space, offset from g-centre back to top-left)
    svg.append('text')
        .attr('data-epv-title', '1')
        .attr('x', -(margin.left + W / 2) + 4)
        .attr('y', -(margin.top + R + 10) + 16)
        .attr('text-anchor', 'start')
        .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
        .text('Sun Chart (Stereographic)');

    // ---- Projection helpers ----
    // Orthographic-style: r = R * cos(alt_rad)  → zenith=0, horizon=R
    function altToR(altDeg) { return R * Math.cos(altDeg * Math.PI / 180); }
    // Azimuth: 0=North going clockwise
    function azmToXY(azmDeg, r) {
      var a = (azmDeg - 90) * Math.PI / 180;  // rotate so 0° (N) is at top
      return { x: r * Math.cos(a), y: r * Math.sin(a) };
    }

    // ---- Altitude circles ----
    var altCircles = [0, 10, 20, 30, 40, 50, 60, 70, 80];
    altCircles.forEach(function(alt) {
      var r = altToR(alt);
      svg.append('circle')
          .attr('r', r)
          .attr('fill', 'none')
          .attr('stroke', '#ccc')
          .attr('stroke-width', alt === 0 ? 1.5 : 0.75)
          .attr('stroke-dasharray', alt === 0 ? '' : '3,3');
      // Label
      if (alt > 0 && alt < 80) {
        svg.append('text')
            .attr('x', 3)
    ...
    // ---- Azimuth radial lines & labels ----
    var azmLines = [
      { a: 0,   lbl: 'N'   }, { a: 45,  lbl: 'NE'  }, { a: 90,  lbl: 'E'   },
      { a: 135, lbl: 'SE'  }, { a: 180, lbl: 'S'    }, { a: 225, lbl: 'SW'  },
      { a: 270, lbl: 'W'   }, { a: 315, lbl: 'NW'   }
    ];
    azmLines.forEach(function(al) {
      var p1 = azmToXY(al.a, 0);
      var p2 = azmToXY(al.a, R);
      svg.append('line')
          .attr('x1', p1.x).attr('y1', p1.y)
          .attr('x2', p2.x).attr('y2', p2.y)
          .attr('stroke', '#ddd')
          .attr('stroke-width', 0.75);
      var pl = azmToXY(al.a, R + 14);
      svg.append('text')
          .attr('x', pl.x)
          .attr('y', pl.y + 4)
          .attr('text-anchor', 'middle')
          .attr('dominant-baseline', 'middle')
          .style('font-size', '11px')
          .style('font-weight', al.lbl === 'N' || al.lbl === 'S' || al.lbl === 'E' || al.lbl === 'W' ? 'bold' : 'normal')
          .attr('fill', '#555')
          .text(al.lbl);
    });

    // ---- Solstice / Equinox day arcs ----
    var dayArcs = [
      { month: 6,  day: 21 },
      { month: 3,  day: 21 },
      { month: 12, day: 21 }
    ];
    var arcLineGen = d3.svg.line()
        .x(function(d) { return azmToXY(d.azm, altToR(d.alt)).x; })
        .y(function(d) { return azmToXY(d.azm, altToR(d.alt)).y; })
        .interpolate('cardinal');
    dayArcs.forEach(function(arc) {
      var pts = allDaytime.filter(function(d) {
        return d.month === arc.month && d.day === arc.day;
    ...
    // ---- Determine variable color scale ----
    var useVar   = (varId !== 'none');
    var colorScale = null;
    var varMin, varMax;

    if (useVar) {
      var vals = allDaytime.map(function(d) { return d[varId]; })
          .filter(function(v) { return v !== null && isFinite(v); });
      if (vals.length) {
        varMin = Math.min.apply(Math, vals);
        varMax = Math.max.apply(Math, vals);
        var hmColors = vDef.colors || getHmColors();
        var nC = hmColors.length;
        var domain = [];
        for (var ci = 0; ci < nC; ci++) {
          domain.push(varMin + (varMax - varMin) * ci / (nC - 1));
        }
        colorScale = d3.scale.linear().domain(domain).range(hmColors);
      } else {
        useVar = false;
      }
    }
    ...
    // ---- Filter active set ----
    // activeSet is already a Set<number> of index values (built in updateSunChart), or null
    var activeIndices = activeSet || null;

    // ---- Tooltip element ----
    var tooltip = d3.select('#epw-sun-tooltip');

    // ---- Plot points ----
    // Draw inactive (gray) first, then active on top
    var inactive = activeIndices
        ? allDaytime.filter(function(d) { return !activeIndices.has(d.index); })
        : [];
    var active   = activeIndices
        ? allDaytime.filter(function(d) { return activeIndices.has(d.index); })
        : allDaytime;

    function plotPoints(pts, isActive) {
      var opac = isActive ? 0.8 : 0.25;
      pts.forEach(function(d) {
        var r  = altToR(d.alt);
        var xy = azmToXY(d.azm, r);
        var c  = isActive
            ? (useVar && colorScale ? colorScale(d[varId]) : color)
            : '#aaa';
        svg.append('circle')
            .datum(d)
            .attr('class', 'sun-pt')
            ...
            .attr('stroke', 'none')
            .on('mouseover', function() {
              var content = '<strong>' + MONTH_NAMES[d.month - 1] + ' ' + d.day + '</strong>&nbsp;' + HOUR_LABELS[d.hour - 1] + '<br>'
                  + 'Altitude: ' + d.alt.toFixed(1) + '°<br>'
                  + 'Azimuth: '  + d.azm.toFixed(1) + '°';
              if (useVar && d[varId] !== null && isFinite(d[varId])) {
                content += '<br>' + vDef.label + ': ' + (vDef.fmt ? vDef.fmt(d[varId]) : d[varId].toFixed(1)) + (vDef.unit ? ' ' + vDef.unit : '');
              }
    ...
    if (inactive.length) plotPoints(inactive, false);
    plotPoints(active, true);

    // ---- Color bar legend (variable mode) ----
    if (useVar && colorScale) {
      var barH = 160, barW = 14;
      var bx = R + 30, by = -barH / 2;
      var colorBarG = svg.append('g').attr('data-epv-legend', '1');
      if (vDef.catLabels) {
        var nCat = vDef.catLabels.length;
        var swH = barH / nCat;
        for (var ci = 0; ci < nCat; ci++) {
          var catIdx = nCat - 1 - ci;
          colorBarG.append('rect')
              .attr('x', bx).attr('y', by + ci * swH)
              .attr('width', barW).attr('height', swH - 0.5)
              .attr('fill', vDef.colors[catIdx]);
          colorBarG.append('text')
              .attr('x', bx + barW + 4)
              .attr('y', by + ci * swH + swH / 2)
              .attr('dy', '0.35em')
              .style('font-size', '9px')
              .attr('fill', '#333')
              .text(vDef.catLabels[catIdx]);
        }
      } else {
        var nSteps = 6;
        var stepH = barH / nSteps;
        for (var si = 0; si < nSteps; si++) {
          var v = varMin + (varMax - varMin) * (nSteps - 1 - si) / (nSteps - 1);
          colorBarG.append('rect')
              .attr('x', bx).attr('y', by + si * stepH)
              .attr('width', barW).attr('height', stepH)
              .attr('fill', colorScale(v));
        }
        for (var ti = 0; ti <= nSteps; ti++) {
          var tv = varMin + (varMax - varMin) * (nSteps - ti) / nSteps;
          colorBarG.append('text')
              .attr('x', bx + barW + 4)
              .attr('y', by + ti * stepH + 4)
              .style('font-size', '10px')
              .attr('fill', '#333')
              .text(vDef.fmt ? vDef.fmt(tv) : tv.toFixed(1));
        }
      }
      colorBarG.append('text')
          .attr('x', bx + barW / 2)
    ...
    var total     = allDaytime.length;
    var selected  = active.length;
    var pct       = total > 0 ? (selected / total * 100).toFixed(1) : '0.0';
    // Place label at upper-left of the SVG (origin is chart centre, so offset by -W/2 and -(R + margin.top - 4))
    var infoGroup = svgEl.append('g')
        .attr('class', 'legend-header')
        .attr('data-epv-datalabel', '1')
        .attr('transform', 'translate(4,' + (svgH - 6) + ')');
    infoGroup.append('text')
        .attr('y', 0)
        .style('font-size', '12px')
        .style('fill', '#555')
        .style('text-anchor', 'start')
        .text(selected + ' of ' + total + ' daytime hours (' + pct + '%) selected');
  }

  // ---- Cartesian Sun Chart ----
  function drawCartesianSunChart(activeSet) {
    var container = d3.select('#epwSunCartesian');
    container.select('svg').remove();
    ...
    var lat = parseFloat(epw.latitude) || 0;
    var southernHemisphere = lat < 0;

    var margin = { top: 30, right: 220, bottom: 70, left: 60 };
    var svgW = 720, svgH = 440;
    var W = svgW - margin.left - margin.right;
    var H = svgH - margin.top - margin.bottom;

    var xScale = d3.scale.linear().domain([0, 360]).range([0, W]);
    var yScale = d3.scale.linear().domain([0, 90]).range([H, 0]);
    // Southern hemisphere: remap azimuth so N (0°) plots at centre and S (180°) at edges.
    // Transform: display_azm = (azm + 180) % 360
    // This gives left→right: S, SW, W, NW, N, NE, E, SE, S
    var azmToX = southernHemisphere
        ? function(a) { return xScale((a + 180) % 360); }
        : xScale;
    ...
    // SVG-embedded title (offset from g origin back to top-left of SVG)
    svg.append('text')
        .attr('data-epv-title', '1')
        .attr('x', -margin.left + 4)
        ...
        .attr('text-anchor', 'start')
        .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
        .text('Sun Chart (Cartesian)');
    [10, 20, 30, 40, 50, 60, 70, 80].forEach(function(alt) {
      svg.append('line')
          .attr('x1', 0).attr('y1', yScale(alt))
          .attr('x2', W).attr('y2', yScale(alt))
          .attr('stroke', '#ddd').attr('stroke-width', 0.75)
          .attr('stroke-dasharray', '3,3');
    });
    [45, 90, 135, 180, 225, 270, 315].forEach(function(azm) {
      svg.append('line')
          .attr('x1', azmToX(azm)).attr('y1', 0)
          .attr('x2', azmToX(azm)).attr('y2', H)
          .attr('stroke', '#ddd').attr('stroke-width', 0.75)
          .attr('stroke-dasharray', '3,3');
    });

    // ---- Axes ----
    var compassTicks  = [0, 45, 90, 135, 180, 225, 270, 315, 360];
    // Southern hemisphere: axis ticks are display positions after (azm+180)%360 remap
    // N (true 0°) → display 180 (centre); S (true 180°) → display 0/360 (edges)
    var compassLabels = southernHemisphere
        ? ['S', 'SW', 'W', 'NW', 'N', 'NE', 'E', 'SE', 'S']
        : ['N', 'NE', 'E', 'SE', 'S', 'SW', 'W', 'NW', 'N'];
    var xAxis = d3.svg.axis().scale(xScale).orient('bottom')
        .tickValues(compassTicks)
        .tickFormat(function(d) { return compassLabels[compassTicks.indexOf(d)]; });
    ...
    svg.append('rect')
        ...
        .attr('fill', 'none').attr('stroke', '#999').attr('stroke-width', 0.75);

    // Axis labels
    svg.append('text')
        .attr('x', W / 2).attr('y', H + 40)
        .attr('text-anchor', 'middle')
        .style('font-size', '11px').attr('fill', '#555')
        .text('Azimuth (°)');
    svg.append('text')
        .attr('transform', 'rotate(-90)')
    ...
    // ---- Day arcs ----
    var cartArcLine = d3.svg.line()
        .x(function(d) { return azmToX(d.azm); })
        .y(function(d) { return yScale(d.alt); })
        .interpolate('cardinal');

    // Solstices + equinox (darker dashed)
    [{ month: 6, day: 21 }, { month: 3, day: 21 }, { month: 12, day: 21 }]
    .forEach(function(arc) {
      ...
      if (!pts.length) return;
      svg.append('path')
          .datum(pts).attr('d', cartArcLine)
          .attr('fill', 'none').attr('stroke', '#888')
          .attr('stroke-width', 1.5).attr('stroke-opacity', 0.7)
          .attr('stroke-dasharray', '5,3');
    });

    // Other months Jan, Feb, Apr, May 21sts (lighter dashed)
    [{ month: 1, day: 21 }, { month: 2, day: 21 }, { month: 4, day: 21 }, { month: 5, day: 21 }]
    .forEach(function(arc) {
      var pts = allDaytime.filter(function(d) {
        return d.month === arc.month && d.day === arc.day;
      }).sort(function(a, b) { return a.hour - b.hour; });
    ...
    function plotPts(pts, isActive) {
      var opac = isActive ? 0.8 : 0.25;
      pts.forEach(function(d) {
        svg.append('circle')
            .datum(d)
            .attr('class', 'sun-pt')
            .attr('cx', azmToX(d.azm))
            .attr('cy', yScale(d.alt))
            .attr('r', 2.5)
            .attr('fill', isActive ? (useVar && colorScale ? colorScale(d[varId]) : color) : '#aaa')
            .attr('opacity', opac)
            .attr('stroke', 'none')
            .on('mouseover', function() {
              var content = '<strong>' + MONTH_NAMES[d.month - 1] + ' ' + d.day + '</strong>&nbsp;' + HOUR_LABELS[d.hour - 1] + '<br>'
    ...
    if (inactive.length) plotPts(inactive, false);
    plotPts(active, true);

    // ---- Color bar legend ----
    if (useVar && colorScale) {
      var barH = 160, barW = 14;
      var bx = W + 30, by = (H - barH) / 2;
      var colorBarG = svg.append('g').attr('data-epv-legend', '1');
      if (vDef.catLabels) {
        var nCat = vDef.catLabels.length;
        var swH = barH / nCat;
        for (var ci = 0; ci < nCat; ci++) {
          ...
          colorBarG.append('rect')
              ...
              .attr('fill', vDef.colors[catIdx]);
          colorBarG.append('text')
              .attr('x', bx + barW + 4).attr('y', by + ci * swH + swH / 2)
              .attr('dy', '0.35em')
              .style('font-size', '9px').attr('fill', '#333')
              .text(vDef.catLabels[catIdx]);
        }
      } else {
        var nSteps = 6;
        var stepH = barH / nSteps;
        for (var si = 0; si < nSteps; si++) {
          var bv = varMin + (varMax - varMin) * (nSteps - 1 - si) / (nSteps - 1);
          colorBarG.append('rect')
              .attr('x', bx).attr('y', by + si * stepH)
              .attr('width', barW).attr('height', stepH)
              .attr('fill', colorScale(bv));
        }
        for (var ti = 0; ti <= nSteps; ti++) {
          var tv = varMin + (varMax - varMin) * (nSteps - ti) / nSteps;
          colorBarG.append('text')
              .attr('x', bx + barW + 4).attr('y', by + ti * stepH + 4)
              .style('font-size', '10px').attr('fill', '#333').text(vDef.fmt ? vDef.fmt(tv) : tv.toFixed(1));
        }
      }
      colorBarG.append('text')
          .attr('x', bx + barW / 2).attr('y', by - 6)
          .attr('text-anchor', 'middle').style('font-size', '10px').attr('fill', '#333')
          .text(vDef.unit || '');
      colorBarG.append('text')
          .attr('x', bx + barW / 2).attr('y', by + barH + 14)
          .attr('text-anchor', 'middle').style('font-size', '10px').attr('fill', '#555')
          .text(vDef.label);
    }
  ...
  // ---- Fast filter-only update for polar sun chart ----
  // Recolours existing circles and updates the info text without rebuilding the SVG.
  function updateSunChartFilter(activeSet) {
    var container = d3.select('#epwSunChart');
    var varId  = window._sunVarId || 'none';
    var vDef   = varDefs.filter(function(v) { return v.id === varId; })[0] || varDefs[0];
    var color  = window._plotColor || 'steelblue';
    ...
    var colorScale = null;
    if (useVar) {
      var vals = allDaytime.map(function(d) { return d[varId]; }).filter(function(v) { return v !== null && isFinite(v); });
      if (vals.length) {
        var varMin = Math.min.apply(Math, vals);
        var varMax = Math.max.apply(Math, vals);
        var hmColors = vDef.colors || getHmColors();
        var nC = hmColors.length;
        var domain = [];
        for (var ci = 0; ci < nC; ci++) { domain.push(varMin + (varMax - varMin) * ci / (nC - 1)); }
        colorScale = d3.scale.linear().domain(domain).range(hmColors);
      } else { useVar = false; }
    }
    container.select('svg').selectAll('circle.sun-pt').each(function(d) {
      if (!d) return;
      var isActive = !activeSet || activeSet.has(d.index);
      d3.select(this)
          .attr('fill', isActive ? (useVar && colorScale ? colorScale(d[varId]) : color) : '#aaa')
          .attr('opacity', isActive ? 0.8 : 0.25);
    });
    // Update info text
    var active   = activeSet ? allDaytime.filter(function(d) { return activeSet.has(d.index); }) : allDaytime;
    var total    = allDaytime.length;
    var selected = active.length;
    var pct      = total > 0 ? (selected / total * 100).toFixed(1) : '0.0';
    container.select('svg').select('[data-epv-datalabel] text')
        .text(selected + ' of ' + total + ' daytime hours (' + pct + '%) selected');
  }

  // ---- Fast filter-only update for cartesian sun chart ----
  function updateCartesianSunChartFilter(activeSet) {
    var container = d3.select('#epwSunCartesian');
    var varId  = window._sunVarId || 'none';
    var vDef   = varDefs.filter(function(v) { return v.id === varId; })[0] || varDefs[0];
    var color  = window._plotColor || 'steelblue';
  ...
  // ---- Crossfilter update hook ----
  // Track last varId to detect variable-vs-filter-only changes
  var _sunLastVarId = null;
  var _sunCartLastVarId = null;

  window.updateSunChart = function(filteredCFData) {
    var activeSet = new Set();
    filteredCFData.forEach(function(d) { activeSet.add(d.index); });
    window._sunLastActive = activeSet;
    var newVarId = window._sunVarId || 'none';
    // Only do full SVG rebuild when variable changed; otherwise just recolour + recount
    if (newVarId !== _sunLastVarId) {
      drawSunChart(activeSet);
      drawCartesianSunChart(activeSet);
      _sunLastVarId     = newVarId;
      _sunCartLastVarId = newVarId;
    } else {
      updateSunChartFilter(activeSet);
      updateCartesianSunChartFilter(activeSet);
    }
  };

  // Initial render
  _sunLastVarId     = window._sunVarId || 'none';
  _sunCartLastVarId = window._sunVarId || 'none';
  drawSunChart(null);
  drawCartesianSunChart(null);
}
