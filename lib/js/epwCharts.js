...
// Maps a UTCI value [°C] to a stress category index 0–9 (coldest → hottest).
...
function _utciCategoryIndex(v) {
    ...
    if (v <   26) return 5;  // no thermal stress
    if (v <   32) return 6;  // moderate heat stress
    if (v <   38) return 7;  // strong heat stress
    if (v <   46) return 8;  // very strong heat stress
    return 9;                // extreme heat stress
}
...
function epwVarDefs(isIP) {
    var defs = [
        ...
        { id: 'utciCategoryIndex',           label: 'UTCI Stress Category',
          ...
          colors: ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'],
...
function epwHeatmapCharts(epw) {
    ...
    { id: 'utciCategoryIndex',
      ...
      catLabels: _utciCatNames,
      steps: 9,
      // Rainbow: violet(0=extreme cold) → blue → cyan → green(5=no stress) → yellow → orange → red(9=extreme heat)
      colors: ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'],
      autoMinMax: false, fixMin: 0, fixMax: 9 }
    ];
...
function epwValueFilter(epw) {
    ...
    var utciCatLabels = ['Extreme Cold','Very Strong Cold','Strong Cold','Moderate Cold','Slight Cold','No Thermal Stress','Moderate Heat','Strong Heat','Very Strong Heat','Extreme Heat'];
    var utciCatColors = ['#7b2abf','#2e4dc2','#0088cc','#00b8d0','#00c880','#00aa44','#88cc00','#eedd00','#f07800','#cc1122'];
...
function epwHCatChart() {
    ...
    function chart(div) {
        div.each(function() {
            ...
            // Display order: lowest key (coldest stress) at top, highest key at bottom.
...
function epwScatterChart(epw) {
    ...
    // Chart geometry — left margin widens for categorical y-axis (single-line labels ~105px)
    var margin = { top: 34, right: 30, bottom: 60, left: 110 };
    var width  = 720;
    var height = 430;
...
function epwTimeseriesCharts(epw) {
    ...
    function drawAnnualTimeseries(activeData, varDef) {
        ...
        var margin  = { top: 34, right: 20, bottom: 40, left: varDef.catLabels ? 116 : 62 };
        var pxPerDay = 2.5;
    ...
    function drawDailyProfiles(activeData, varDef) {
        ...
        var panelInnerH  = 170;
        var panelGutter  = 4;
        var marginLeft   = varDef.catLabels ? 110 : 56;   // space for shared y-axis
...
function epwDurationCurve(epw) {
  ...
  function draw(activeData) {
    ...
    var margin = { top: 24, right: 165, bottom: 60, left: vDef.catLabels ? 110 : 70 };
