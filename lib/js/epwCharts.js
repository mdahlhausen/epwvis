...
function epwScatterChart(epw) {
    ...
    // Chart geometry
    var margin = { top: 34, right: 30, bottom: 60, left: 70 };
    var width  = 720;
    var height = 430;
    ...
    function redrawAxes() {
        ...
        yAxisG.call(d3.svg.axis().scale(y).orient('left').ticks(8).tickFormat(function(v) { return yVd.fmt ? yVd.fmt(v) : v; }));
...
function epwTimeseriesCharts(epw) {
    ...
    function drawAnnualTimeseries(activeData, varDef) {
        ...
        var margin  = { top: 34, right: 20, bottom: 40, left: 62 };
        var pxPerDay = 2.5;
...
function epwDurationCurve(epw) {
  ...
  function draw(activeData) {
    ...
    svg.append('text')
        ...
        .attr('text-anchor', 'start')
        .style('font-size', '13px').style('font-weight', 'bold').style('fill', '#333')
        .text(vDef.label + ' (' + getUnit(vDef) + ') — Duration Curve');

    // Axes
    var xAxis = d3.svg.axis().scale(xScale).orient('bottom').ticks(8);
    var yAxis = d3.svg.axis().scale(yScale).orient('left').ticks(8).tickFormat(function(v) { return vDef.fmt ? vDef.fmt(v) : v.toFixed(1); });
