...
function epwHeatmap(data,params) {
	...
	var margin = {top: 10, right: params.catLabels ? 160 : 120, bottom: 40, left: 40},
...
function epwValueFilter(epw) {
    ...
    function renderAll() {
      ...
      if (window.updateTimeseries)     { window.updateTimeseries(filteredData); }
      ...
      if (window.updateDurationCurve)  { window.updateDurationCurve(filteredData); }
      if (window.updateDegreeDays)     { window.updateDegreeDays(filteredData); }
      if (window.updateSunChart)       { window.updateSunChart(filteredData); }
    }
...
function epwHCatChart() {
    ...
    var margin = { top: 6, right: 20, bottom: 26, left: 148 };
    var barH   = 20;
    var barGap = 3;
    ...
    function chart(div) {
        div.each(function() {
            ...
            if (svg.empty()) {
                ...
                gRoot.append("g").attr("class", "hcat-count-labels");
            ...
            // Pre-compute per-bar metadata so each attribute callback shares the same values.
            var totalActive = d3.sum(_bgData, function(d) { return fgByKey[d.key] || 0; });
            var cntMeta = {};
            _bgData.forEach(function(d) {
                var cnt    = fgByKey[d.key] || 0;
                var pct    = totalActive > 0 ? (cnt / totalActive * 100).toFixed(1) : '0.0';
                var lbl    = cnt + ' hrs (' + pct + '%)';
                var barPx  = xScale(cnt);
                var estW   = lbl.length * 6;   // ~6px per char at 10px sans-serif
                cntMeta[d.key] = { cnt: cnt, lbl: lbl, barPx: barPx, inside: barPx >= estW + 8 };
            });

            var cntLbls = g.select(".hcat-count-labels")
                .selectAll("text.hcat-cnt").data(_bgData, function(d) { return d.key; });
            cntLbls.enter().append("text").attr("class", "hcat-cnt");
            cntLbls
                .attr("x", function(d) {
                    var m = cntMeta[d.key];
                    return m.inside ? m.barPx - 4 : m.barPx + 4;
                })
                .attr("y", function(d) { return yPos(d.key) + barH / 2; })
                .attr("dy", "0.35em")
                .attr("text-anchor", function(d) { return cntMeta[d.key].inside ? "end" : "start"; })
                .style("font-size", "10px")
                .style("fill", function(d) {
                    var m = cntMeta[d.key];
                    if (m.inside) return "white";
                    if (selectedKeys && !selectedKeys.has(d.key)) return "#bbb";
                    return "#444";
                })
                .text(function(d) { return cntMeta[d.key].lbl; });
            cntLbls.exit().remove();
...
function epwSunChart(epw) {
  ...
  function drawSunChart(activeSet) {
    ...
    var infoGroup = svg.append('g')
        .attr('class', 'legend-header')
        .attr('transform', 'translate(' + (-(W / 2)) + ',' + (-(R + margin.top - 16)) + ')');
    infoGroup.append('text')
        .attr('y', 0)
        .style('text-anchor', 'start')
