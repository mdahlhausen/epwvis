    ...
    function renderHeatmaps() {
        ...
        hmVarDefs.forEach(function(vd) {
            ...
            epwHeatmap(epwData(epw, cleanVals), {
                ...
                unit:        unitLbl,
                steps:       vd.steps,
                step_colors: vd.colors
            });

            // Build and cache the same color scale so the update hook can recolour cells
            var scaleStep = (maxVal - minVal) / vd.steps;
            var colorVals = [];
            for (var ci = 0; ci <= vd.steps; ci++) { colorVals.push(minVal + scaleStep * ci); }
            window._hmColorScales[divId] = d3.scale.linear().domain(colorVals).range(vd.colors);
        });
...
function epwValueFilter(epw) {
    ...
    var vfVarList = [
        ...
        { divId: 'pressure-chart',             chartIdx: 13, label: 'Station Pressure',            defaultOn: false }
    ];
...
function epwTabularData(epw) {
    ...
    function renderTable(activeData, varId) {
        ...
        for (var m = 1; m <= 12; m++) {
            ...
            var row = [MONTH_NAMES[m - 1], fmt(s.max), fmt(s.min), fmt(s.mean), fmt(s.std), fmt(s.p1), fmt(s.p99)];
            rowData.push(row);
            html += '<tr><td>' + row.join('</td><td>') + '</td></tr>';
        }
        var sAll = computeStats(allVals);
        var yearRow = ['Year', fmt(sAll.max), fmt(sAll.min), fmt(sAll.mean), fmt(sAll.std), fmt(sAll.p1), fmt(sAll.p99)];
...
function epwTimeseriesCharts(epw) {
    ...
    function drawAnnualTimeseries(activeData, varDef) {
        ...
        g.append('path')
            ...
            .style('fill', 'steelblue').style('fill-opacity', 0.35);
        ...
        g.append('path')
            ...
            .style('fill', 'none').style('stroke', 'steelblue').style('stroke-width', '2px');
    ...
    function drawDailyProfiles(activeData, varDef) {
        ...
        for (var mi = 1; mi <= 12; mi++) {
            (function(m) {
                ...
                // Month name above panel, colored
                g.append('text')
