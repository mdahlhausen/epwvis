...
(function (root) {
  ...
  // ---- Solar Position: compute altitude (°) and azimuth (°) for all EPW hours ----
  // Returns { altitude: [], azimuth: [] } parallel to epw hours array
  // altitude: degrees above horizon (negative = below); azimuth: 0=N, 90=E, 180=S, 270=W
  function computeSolarPositions(epw) {
    var lat = parseFloat(epw.latitude);
    var lon = parseFloat(epw.longitude);
    var tz  = parseFloat(epw.timeZone);
    var years  = epw.year();
    ...
    var hours  = epw.hour();
    var n = years.length;
    var altitude = new Array(n);
    var azimuth  = new Array(n);
    for (var i = 0; i < n; i++) {
      // Use mid-hour convention (hr-1 + 30 min) consistent with PVWatts
      var sol = solarpos(years[i], months[i], days[i], hours[i] - 1, 30, lat, lon, tz);
      altitude[i] = (Math.PI / 2 - sol.zen) / DTOR;  // elevation in degrees
      azimuth[i]  = sol.azm / DTOR;                    // azimuth in degrees (N=0, clockwise)
    }
  ...
  var SolarPV = {
    ...
    computeSolarPositions: computeSolarPositions,
