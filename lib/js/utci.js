 ...
 *
 * EPW fields used (0-indexed after 8 header rows are stripped):
 *   row[1]  month
 *   row[2]  day
 *   row[3]  hour (1–24, EPW convention)
 *   row[6]  dry-bulb temperature [°C]
 *   row[8]  relative humidity [%]
 *   row[14] direct normal radiation [Wh/m²]
 *   row[21] wind speed at 10 m [m/s]
 *
 * Site header values used:
 *   epw.latitude   decimal degrees (+N)
 *   epw.longitude  decimal degrees (+E)
 *   epw.timeZone   UTC offset in hours (+E)
 *-------------------------------------------------------------------------*/
...
/**
 * Day-of-year (1-indexed) from month and day.
 * Assumes a non-leap year (standard for TMY EPW files).
 *
 * @param {number} month - Month [1–12]
 * @param {number} day   - Day of month [1–31]
 * @returns {number} Day of year [1–365]
 */
function _utciDayOfYear(month, day) {
  var cum = [0, 0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334];
  return cum[month] + day;
}

/**
 * Solar altitude angle above the horizon [degrees], clamped to ≥ 0.
 *
 * Uses the Spencer (1971) formula for declination and equation of time.
 * Accuracy ≈ ±0.5°, sufficient for thermal comfort calculations.
 *
 * @param {number} lat      - Site latitude [decimal degrees, +N]
 * @param {number} lon      - Site longitude [decimal degrees, +E]
 * @param {number} timezone - UTC offset [hours, +E]
 * @param {number} month    - Month [1–12]
 * @param {number} day      - Day of month [1–31]
 * @param {number} hour     - EPW hour [1–24]; time-step midpoint = hour − 0.5
 * @returns {number} Solar altitude [degrees, ≥ 0; 0 at night]
 */
function utciSolarAltitudeDegrees(lat, lon, timezone, month, day, hour) {
  var doy = _utciDayOfYear(month, day);
  ...
  // True solar time [hours]; EPW hour midpoint = hour − 0.5
  var lst       = hour - 0.5;
  var solarTime = lst + eot / 60.0 + (lon - timezone * 15.0) / 15.0;

  // Hour angle [radians]: 0 at solar noon, ± 15°/hr
  var ha = (solarTime - 12.0) * 15.0 * Math.PI / 180.0;
  ...
  return Math.max(0, Math.asin(sinAlt) * 180.0 / Math.PI);
}


/*==========================================================================
 * 2. SOLAR GAIN (ASHRAE 55 ERF / delta-MRT model)
 *==========================================================================*/
